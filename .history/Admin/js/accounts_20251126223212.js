...
async function showStaffActivityModal(hoten, datphongs, thanhtoans, denbus) {
    ...
    bookingBody.innerHTML = datphongs.length ? datphongs.map(dp => {
        let statusColor = '#ffc107'; // warning (vàng)
        if (dp.trangthai && dp.trangthai.includes('Đã đặt')) {
            statusColor = '#4caf50'; // success (xanh lá)
        } else if (dp.trangthai && dp.trangthai.includes('Đã hủy')) {
            statusColor = '#f44336'; // danger (đỏ)
        }
        return `
        <tr>
            ...
            ">${dp.trangthai}</span></td>
            <td class="text-end fw-bold">${formatMoney(dp.tongtien)}</td>
        </tr>
        `;
    }).join('') : '<tr><td colspan="6" class="text-center text-muted py-4">Chưa có đặt phòng</td></tr>';
