...
function setupFilters() {
    ...
    applyBtn.addEventListener('click', async() => {
        if (isLoading) return;
        let start, end;
        const preset = document.querySelector('#preset-filter-options .custom-select-option.active') ? .dataset.value || 'this-month';
...
function renderTopRoomTypes(roomTypes) {
    const ctx = document.getElementById('topRoomTypesChart') ? .getContext('2d');
    if (!ctx) {
            ...
            <div class="review-header">
                <div>
                    ...
                    <span class="room-info">
                        Mã đặt phòng <strong>${r.madatphong}</strong>
                    </span>
                </div>
            </div>
