...
.map(b => {
    ...
    const isPaid = ['ĐÃ THANH TOÁN', 'Chưa thanh toán', 'Đã xử lý'].includes(trangThai);
    ...
    try {
        ...
        return {
            ...
            paymentStatus: (() => {
                ...
                if (tt === 'Đã xử lý') return 'refunded';
                return 'unpaid';
            })(),
...
if (query && query.trim() !== '') {
    ...
    result = invoices.filter(inv => {
        ...
        return result.map(inv => {
            ...
            else if (tt.includes('Đã xử lý')) paymentStatus = 'refunded';
...
export async function fetchInvoiceDetails(id) {
    try {
        ...
        const paymentStatus = trangThai === 'Đã thanh toán' ? 'paid' : 
                             trangThai === 'Đã xử lý' ? 'refunded' : 'unpaid';
