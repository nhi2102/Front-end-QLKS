...
function displayAllBookingInfo(bookingData) {
    setTimeout(() => {
        ...
        displayLoyaltyPoints(bookingData); // Hiển thị điểm tích lũy
    }, 100);
}

// Hiển thị điểm thành viên hiện tại của khách hàng
// Lưu ý: Điểm sẽ được cập nhật TỰ ĐỘNG bởi trigger TRG_UPDATETICHDIEM 
// khi checkout (TRANGTHAI = 'Đã trả'): DIEMTHANHVIEN += TONGTIEN * 0.001
async function displayLoyaltyPoints(bookingData) {
    ...
    try {
        ...
        if (!makhachhang) {
            console.warn(' Không tìm thấy mã khách hàng để lấy điểm tích lũy');
            return;
        }

        // Fetch thông tin khách hàng để lấy điểm tích lũy
        const response = await fetch(`https://localhost:7076/api/Khachhangs/${makhachhang}`);
        if (!response.ok) {
            console.error('Lỗi khi fetch thông tin khách hàng:', response.status);
            return;
        }
        ...
        console.log(' Điểm thành viên hiện tại:', currentPoints);

        // Tính điểm sẽ nhận (theo trigger: TONGTIEN * 0.001)
        ...
        if (pointsToEarn > 0) {
            infoElement.innerHTML = `<i class="fas fa-gift"></i><span>Nhận <strong>+${pointsToEarn}</strong> điểm sau khi check-out</span>`;
        } else {
            infoElement.innerHTML = `<i class="fas fa-info-circle"></i><span>Nhận điểm sau khi check-out</span>`;
        }

        // Setup áp dụng điểm
        setupApplyPoints(currentPoints, totalPrice);

        // Hiển thị container
        loyaltyContainer.style.display = 'block';

    } catch (error) {
        console.error(' Lỗi khi hiển thị điểm tích lũy:', error);
    }
}

// Setup chức năng áp dụng điểm để giảm giá
function setupApplyPoints(availablePoints, totalPrice) {
    ...
    if (!checkbox || !pointsInput || !discountPreview) return;

    // Tỷ lệ quy đổi: 1 điểm = 1,000 VNĐ
    const POINT_TO_VND = 1000;

    // Số điểm tối đa có thể dùng (không vượt quá tổng tiền)
    const maxUsablePoints = Math.min(availablePoints, Math.floor(totalPrice / POINT_TO_VND));
    ...
    checkbox.addEventListener('change', function() {
        if (this.checked) {
            ...
            if (maxUsablePoints === 0) {
                alert('⚠️ Bạn không có đủ điểm để sử dụng hoặc tổng tiền quá nhỏ.');
                this.checked = false;
            ...
            updateTotalPrice(0); // Reset về giá gốc
        }
    });
    ...
    console.log('⭐ Áp dụng điểm:', { availablePoints, maxUsablePoints, totalPrice });
}

// Cập nhật tổng tiền hiển thị khi áp dụng điểm
function updateTotalPrice(discount) {
    const finalPriceElement = document.querySelector('.final-price');
    if (!finalPriceElement) return;

    // Lấy giá gốc từ localStorage
    ...
    if (discount > 0) {
        finalPriceElement.innerHTML = `
            ...
            <span style="color: #28a745; font-size: 18px; font-weight: bold; margin-left: 8px;">
                ${finalPrice.toLocaleString('vi-VN')} VNĐ
            </span>
        `;
    } else {
        finalPriceElement.textContent = `${originalPrice.toLocaleString('vi-VN')} VNĐ`;
    }
}

// Lấy số điểm được sử dụng (để truyền vào booking)
function getUsedPoints() {
    const checkbox = document.getElementById('use-points-checkbox');
    const pointsInput = document.getElementById('points-to-use');
    
    if (checkbox && checkbox.checked && pointsInput) {
        return parseInt(pointsInput.value) || 0;
    }
    return 0;
}
...
async function handleFormSubmit(event) {
    ...
    try {
        ...
        // Lấy số điểm được sử dụng
        const usedPoints = getUsedPoints();
        const pointsDiscount = usedPoints * 1000; // 1 điểm = 1,000 VNĐ
        ...
        const completeBookingData = {
            ...
            usedPoints: usedPoints, // Số điểm đã sử dụng
            pointsDiscount: pointsDiscount, // Giảm giá từ điểm
        ...
        console.log('💳 Thông tin thanh toán:', {
            ...
            usedPoints,
            pointsDiscount,
            finalPrice: bookingData.totalPrice - pointsDiscount
        });
        ...
        if (result.success) {
            ...
            // Lưu ý: Điểm thành viên sẽ được cập nhật TỰ ĐỘNG bởi trigger TRG_UPDATETICHDIEM
            // khi checkout (TRANGTHAI = 'Đã trả'): DIEMTHANHVIEN += TONGTIEN * 0.001
            
            // Hiển thị thông báo đặt đoàn (nếu > 5 phòng)
            if (result.isGroupBooking && result.totalRooms > 5) {
                ...
                alert(`🎉 ĐẶT THEO ĐOÀN!\n\n` +
                      `Số phòng: ${result.totalRooms} phòng\n` +
                      `Giảm giá: ${discountPercent}% (${giamGia.toLocaleString('vi-VN')} VNĐ)\n` +
                      `Tổng tiền sau giảm: ${tongTien.toLocaleString('vi-VN')} VNĐ\n\n` +
                      `Cảm ơn quý khách đã tin tưởng đặt phòng theo đoàn!`);
            }
            ...
            // Lấy tổng tiền CHÍNH XÁC từ trigger (đã trừ giảm giá đặt đoàn)
            let tongTien = result.data.tongTien;
            
            // Nếu tongTien = 0 hoặc null, tính từ donGia - giamGia
            if (!tongTien || tongTien === 0) {
                const donGia = result.data.donGia || 0;
                const giamGia = result.data.giamGia || 0;
                tongTien = donGia - giamGia;
                console.warn('⚠️ tongTien = 0/null, tính lại từ donGia - giamGia:', { donGia, giamGia, tongTien });
            }
            
            // Trừ thêm giảm giá từ điểm (nếu có)
            const finalTongTien = tongTien - pointsDiscount;
            
            console.log('💰 Chi tiết thanh toán:', {
                tongTienGoc: tongTien,
                usedPoints: usedPoints,
                pointsDiscount: pointsDiscount,
                finalAmount: finalTongTien
            });
            
            // Lưu thông tin điểm đã dùng để trừ sau khi thanh toán thành công
            if (usedPoints > 0) {
                localStorage.setItem('pendingPointsDeduction', JSON.stringify({
                    maDatPhong,
                    usedPoints,
                    pointsDiscount
                }));
            ...
            if (paymentMethod === 'vnpay') {
                ...
                try {
                    ...
                    console.log(' Gọi createVNPayPayment với params:', {
                        ...
                        amount: finalTongTien, // Đã trừ điểm
                        orderInfo: orderInfo,
                    ...
                    const vnpayUrl = await createVNPayPayment({
                        ...
                        orderId: maHoaDon || maDatPhong,
                        orderType: 'hotel_booking',
                        amount: finalTongTien, // Số tiền đã trừ điểm
                        orderInfo: orderInfo,
                        orderDescription: `Dat phong khach san - ${customerName}`,
                        returnUrl: `${window.location.origin}/payment-result.html`
                    });
