...
function getRoomListFromBooking(booking) {
    ...
    if (booking.danhSachPhong && Array.isArray(booking.danhSachPhong)) {
        booking.danhSachPhong.forEach(room => {
            if (room.sophong) roomSet.add(room.sophong);
        });
    }
...
function handleEquipmentDamage(equipmentId, equipmentName, equipmentPrice) {
    ...
    if (checkbox && roomSelect) {
        // Show/hide room select based on checkbox
        if (checkbox.checked) {
    ...
    try {
        ...
        for (const checkbox of equipmentItems) {
            ...
            const damageData = {
                ...
                lyDo: `Thiết bị ${equipmentName} bị hư hỏng ở phòng ${selectedRoom} khi checkout`,
            ...
            try {
                ...
                if (response.ok) {
                    console.log(` Equipment damage recorded: ${equipmentName} - Room ${selectedRoom}`);
                } else {
