...
async function createBookingAPI(payload) {
    ...
    // Bước 3: tạo hóa đơn tự động
    try {
        await apiFetch(`${API_BASE_URL}/Hoadons/taohoadon/${datphong.madatphong}`, {
            method: "POST"
        });
        console.log(`[Client] Đã tạo hóa đơn cho đơn đặt phòng #${datphong.madatphong}`);
    } catch (err) {
        console.warn("[Client] Không thể tự động tạo hóa đơn:", err);
    }
