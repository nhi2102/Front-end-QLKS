// ============================================
// CHECK-OUT PAGE - JAVASCRIPT (FIXED VERSION)
// ============================================
...
function extractRoomNumber(booking) {
    ...
    if (booking.soPhong) return booking.soPhong;
    if (booking.SoPhong) return booking.SoPhong;
    ...
    if (booking.chitietdatphongs && Array.isArray(booking.chitietdatphongs) && booking.chitietdatphongs.length > 0) {
        ...
        for (const detail of booking.chitietdatphongs) {
            ...
            else if (detail.maphongNavigation && detail.maphongNavigation.sophong) roomNum = detail.maphongNavigation.sophong;
            else if (detail.maphongNavigation && detail.maphongNavigation.soPhong) roomNum = detail.maphongNavigation.soPhong;
            
            if (roomNum) roomNumbers.push(roomNum);
        }
...
function displayCurrentGuests(bookings) {
    ...
    tbody.innerHTML = bookings.map(booking => {
        ...
        const totalAmount = booking.tongTienDatPhong || booking.TongTienDatPhong || booking.tongtien || booking.totalAmount || 0;
...
async function fetchServiceCharges(maDatPhong) {
    try {
        ...
        try {
            ...
            console.warn('⚠️ Could not fetch from Chitiethoadons:', error.message);
        }
        ...
        if (totalServiceAmount === 0) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log('🛎️ Service usage found in Sudungdvs:', bookingServiceUsage.length);
                ...
                console.warn('⚠️ Could not fetch from Sudungdvs:', error.message);
            }
        }
        ...
        if (totalServiceAmount === 0) {
            try {
                ...
                console.warn('⚠️ Could not fetch service history:', error.message);
            }
        }
    ...
    try {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('🛎️ Service history from API:', serviceHistory.length, 'records');
            }
        } catch (error) {
            console.warn('⚠️ Service history API not available:', error.message);
        }
        ...
        if (serviceHistory.length === 0) {
            try {
                ...
                console.warn('⚠️ Could not fetch service usage:', error.message);
            }
        }
...
function displayServiceHistoryInModal(serviceHistory) {
    ...
    if (!serviceHistoryContainer) {
        console.warn('⚠️ Service history container not found in modal');
        return;
    }
...
async function loadEquipmentForDamageCheck() {
    try {
        console.log('🛠️ Loading equipment for damage check...');
...
async function fetchHotelEquipment() {
    try {
        console.log('🛠️ Fetching hotel equipment list...');
        ...
        console.log('🛠️ Equipment loaded:', equipment.length, 'items');
        return equipment;
...
async function confirmCheckOut() {
    ...
    try {
        ...
        try {
            ...
            console.warn('⚠️ Printing invoice failed:', e);
        }
        ...
        if (typeof soTienThieu !== 'number') {
            console.warn('⚠️ Invalid soTienThieu value:', soTienThieu);
        }
        ...
        if (!checkoutResult.message || !checkoutResult.message.includes('thành công')) {
            console.warn('⚠️ Unexpected checkout result:', checkoutResult);
        }

        // Check payment status BEFORE executing checkout
        if (soTienThieu > 0) {
            const shouldProceed = confirm(
                `⚠️ CẢNH BÁO: Khách hàng còn nợ ${formatCurrency(soTienThieu)}\n\n` +
        ...
        if (soTienThieu > 0) {
            successMessage += `⚠️ CẢNH BÁO: CÒN THIẾU TIỀN!\n`;
        ...
        if (error.message.includes('Failed to fetch')) {
            ...
            errorMessage += '✏️ Vui lòng kiểm tra mã đặt phòng và thử lại';
        } else {
...
async function handleEquipmentDamageOnCheckout(bookingId) {
    try {
        ...
        if (equipmentItems.length === 0) {
            console.log('ℹ️ No equipment damage to process');
            return;
        }
