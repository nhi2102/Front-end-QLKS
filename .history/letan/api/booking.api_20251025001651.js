...
async function createBookingAPI(payload) {
    ...
    // Bước 4: in phiếu xác nhận đặt phòng
    const fullBooking = await fetchBookingFullInfo(datphong.madatphong || datphong.id);
    selectedBooking = fullBooking;
    printBooking(selectedBooking);
...
async function fetchBookingFullInfo(bookingId) {
    try {
        ...
        return {
            id: booking.madatphong || booking.MADATPHONG || booking.id || "",
            bookingDate: booking.ngaydat || booking.NGAYDAT || booking.ngaynhanphong || booking.NGAYNHANPHONG || "",
            status: booking.trangthai || booking.TRANGTHAI || "",
            paymentStatus: booking.trangthaithanhtoan || booking.TRANGTHAITHANHTOAN || "",
            customerName: booking.tenkhachhang || booking.TENKHACHHANG || "",
            customerPhone: booking.sodienthoai || booking.SODIENTHOAI || "",
            customerEmail: booking.email || booking.EMAIL || "",
            customerID: booking.cccd || booking.CCCD || "",
            roomNumber: roomNumber || "",
            roomType: roomType || "",
            checkinDate: firstDetail.ngaynhanphong || firstDetail.NGAYNHANPHONG || "",
            checkoutDate: firstDetail.ngaytraphong || firstDetail.NGAYTRAPHONG || "",
            tienPhong: tienPhong || 0,
            tienDichVu: tienDichVu || 0,
