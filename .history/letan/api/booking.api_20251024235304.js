...
async function createBookingAPI(payload) {
    ...
    printBooking({
        ...
        customerName: payload.KhachHang ? .hoten || "Khách lẻ",
        customerPhone: payload.KhachHang ? .sdt || "",
        customerEmail: payload.KhachHang ? .email || "",
        customerID: payload.KhachHang ? .cccd || "",
        roomNumber: payload.Chitietdatphongs ? .[0] ? .SoPhong || "",
        roomType: payload.Chitietdatphongs ? .[0] ? .LoaiPhong || "",
        checkinDate: payload.Ngaynhanphong,
        checkoutDate: payload.Ngaytraphong,
        tienPhong: payload.Chitietdatphongs.reduce((sum, r) => sum + r.Tongcong, 0),
        tienDichVu: 0,
        totalAmount: payload.Chitietdatphongs.reduce((sum, r) => sum + r.Tongcong, 0),
