...
// Extract room number using pending-checkouts API fields
function extractRoomNumber(booking) {
    ...
    // Strategy 3: From chitietdatphongs array (fallback)
    if (booking.chitietdatphongs && Array.isArray(booking.chitietdatphongs)) {
        for (const detail of booking.chitietdatphongs) {
            if (detail.sophong) return detail.sophong;
            if (detail.soPhong) return detail.soPhong;
            if (detail.maphongNavigation && detail.maphongNavigation.sophong) return detail.maphongNavigation.sophong;
        }
    }
