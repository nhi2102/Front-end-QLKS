...
document.addEventListener('DOMContentLoaded', () => {
    ...
    checkUserLogin();
    setupEventListeners();
    setupInputValidation(); // Thêm validation
});

// Hàm kiểm tra trùng lặp khách hàng
async function checkCustomerDuplicate(sdt, cccd, email, excludeId) {
    try {
        // Kiểm tra SĐT
        if (sdt) {
            const sdtResponse = await fetch(`${API_BASE_URL}/Khachhangs/timkiem?sdt=${sdt}`);
            if (sdtResponse.ok) {
                const sdtResult = await sdtResponse.json();
                if (Array.isArray(sdtResult) && sdtResult.length > 0) {
                    const duplicate = sdtResult.find(k => k.makh != excludeId);
                    if (duplicate) return 'Số điện thoại đã được sử dụng bởi khách hàng khác!';
                }
            }
        }
        ...
        // Kiểm tra CCCD
        if (cccd) {
            const cccdResponse = await fetch(`${API_BASE_URL}/Khachhangs/timkiem?cccd=${cccd}`);
            if (cccdResponse.ok) {
                const cccdResult = await cccdResponse.json();
                if (Array.isArray(cccdResult) && cccdResult.length > 0) {
                    const duplicate = cccdResult.find(k => k.makh != excludeId);
                    if (duplicate) return 'Số CCCD/CMND đã được sử dụng bởi khách hàng khác!';
                }
            }
        }
        ...
        // Kiểm tra Email
        if (email) {
            const emailResponse = await fetch(`${API_BASE_URL}/Khachhangs/timkiem?email=${email}`);
            if (emailResponse.ok) {
                const emailResult = await emailResponse.json();
                if (Array.isArray(emailResult) && emailResult.length > 0) {
                    const duplicate = emailResult.find(k => k.makh != excludeId);
                    if (duplicate) return 'Email đã được sử dụng bởi khách hàng khác!';
                }
            }
        }
...
function setupInputValidation() {
    const phoneInput = document.getElementById('editSdt');
...
async function saveCustomer() {
    ...
    const editHotenEl = document.getElementById('editHoten');
    const editCccdEl = document.getElementById('editCccd');
    const editSdtEl = document.getElementById('editSdt');
    const editEmailEl = document.getElementById('editEmail');
    const editNgaysinhEl = document.getElementById('editNgaysinh');
    const editDiachiEl = document.getElementById('editDiachi');
    const editTrangthaiEl = document.getElementById('editTrangthai');

    const customerData = {
        hoten: editHotenEl ? editHotenEl.value.trim() : '',
        cccd: editCccdEl ? editCccdEl.value.trim() : '',
        sdt: editSdtEl ? editSdtEl.value.trim() : '',
        email: editEmailEl ? editEmailEl.value.trim() : '',
        ngaysinh: editNgaysinhEl ? (editNgaysinhEl.value || null) : null,
        diachi: editDiachiEl ? editDiachiEl.value.trim() : '',
        trangthai: editTrangthaiEl ? editTrangthaiEl.value : 'Hoạt động',
        matkhau: (currentCustomer && (currentCustomer.matkhau || currentCustomer.Matkhau)) || "temporary_password"
    };
    ...
    if (!customerData.sdt.startsWith('0')) {
        alert('Số điện thoại phải bắt đầu bằng số 0!');
        return;
    }
    const phoneRegex = /^0[0-9]{9}$/;
    if (!phoneRegex.test(customerData.sdt)) {
    ...
    // Kiểm tra trùng lặp
    const duplicateError = await checkCustomerDuplicate(customerData.sdt, customerData.cccd, customerData.email, isEdit ? customerId : null);
    if (duplicateError) {
    ...
    try {
        ...
        if (isEdit) {
            ...
            console.log("Current customer:", currentCustomer); // debug
...
/**
 * Gọi API để gửi mật khẩu mới qua email.
 */
async function sendNewPasswordByEmail(email) {
    showLoading(true);
    try {
        const response = await fetch(`${API_BASE_URL}/Taikhoans/QuenMatKhau?email=${encodeURIComponent(email)}`, {
            method: "POST"
        });

        if (!response.ok) throw new Error('Không thể gửi mật khẩu mới');

        alert('Mật khẩu mới đã được gửi đến email: ' + email);
    } catch (error) {
        console.error('Lỗi gửi mật khẩu mới:', error);
        alert('Có lỗi xảy ra khi gửi mật khẩu mới: ' + error.message);
    } finally {
        showLoading(false);
    }
}
