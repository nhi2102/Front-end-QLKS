...
// Tạo hóa đơn mới cho dịch vụ
async function createServiceInvoice(bookingId, services) {
    const totalAmount = services.reduce((sum, s) => sum + (s.giatien || s.giaDichVu || s.price) * s.quantity, 0);

    // Kiểm tra hóa đơn hiện có trước
    console.log(' Kiểm tra hóa đơn hiện có cho đặt phòng:', bookingId);
    try {
        // Lấy tất cả hóa đơn và filter theo madatphong
        const checkResponse = await fetch(`${API_BASE}/Hoadons`);
        if (checkResponse.ok) {
            const allInvoices = await checkResponse.json();
            const existingInvoices = allInvoices.filter(inv => inv.madatphong == bookingId);
            if (existingInvoices && existingInvoices.length > 0) {
                console.log(' Sử dụng hóa đơn hiện có:', existingInvoices[0]);
                return existingInvoices[0];
            }
        }
    } catch (checkError) {
        console.log(' Lỗi kiểm tra hóa đơn hiện có:', checkError);
    }

    // Lấy thông tin booking để có makh
    let makh = 1; // Giá trị mặc định
    try {
        const bookingResponse = await fetch(`${API_BASE}/Datphongs/${bookingId}`);
        if (bookingResponse.ok) {
            const booking = await bookingResponse.json();
            makh = booking.makh || 1;
        }
    } catch (e) {
        console.log(' Không lấy được thông tin booking, dùng makh mặc định:', makh);
    }

    // Tạo hóa đơn với cấu trúc đúng theo API
    const invoiceData = {
        ngaylap: new Date().toISOString().split('T')[0], // Format: YYYY-MM-DD
    ...
    console.log(' Tạo hóa đơn dịch vụ mới:', invoiceData);

    // Thử sử dụng endpoint taohoadon trước (stored procedure)
    try {
        const autoResponse = await fetch(`${API_BASE}/Hoadons/taohoadon`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (autoResponse.ok) {
            ...
            console.log(' Hóa đơn tự động đã tạo:', autoResult);

            // Lấy hóa đơn mới nhất cho booking này
            const latestResponse = await fetch(`${API_BASE}/Hoadons`);
            if (latestResponse.ok) {
                const allInvoices = await latestResponse.json();
                const bookingInvoices = allInvoices.filter(inv => inv.madatphong == bookingId);
                if (bookingInvoices && bookingInvoices.length > 0) {
                    return bookingInvoices[bookingInvoices.length - 1];
                }
            }
        }
    } catch (autoError) {
        console.log(' Không thể sử dụng auto create, thử tạo manual:', autoError);
    }

    // Fallback: Tạo hóa đơn manual với payload đơn giản hóa
    let response = await fetch(`${API_BASE}/Hoadons`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    // Nếu lỗi 400, thử với payload đơn giản hơn
    if (!response.ok && response.status === 400) {
        console.log(' Thử với payload đơn giản hơn...');
        ...
        response = await fetch(`${API_BASE}/Hoadons`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(simpleInvoiceData)
        });
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Lỗi tạo hóa đơn (${response.status}): ${errorDetails}`);
    }

    const result = await response.json();
    console.log(' Hóa đơn dịch vụ mới đã tạo:', result);

    return result;
}

// Lấy thông tin chi tiết hóa đơn
async function getInvoiceDetails(mahoadon) {
    try {
        console.log(' Lấy chi tiết hóa đơn:', mahoadon);

        const response = await fetch(`${API_BASE}/Chitiethoadons/hoadon/${mahoadon}`);

        if (!response.ok) {
            console.warn(' Không thể lấy chi tiết hóa đơn:', response.status);
            return [];
        }

        const details = await response.json();
        console.log(' Chi tiết hóa đơn:', details);
...
async function getInvoicesByBooking(madatphong) {
    try {
        console.log(' Lấy hóa đơn theo booking:', madatphong);

        // Lấy tất cả hóa đơn và filter theo madatphong
        const response = await fetch(`${API_BASE}/Hoadons`);

        if (!response.ok) {
            console.warn(' Không thể lấy hóa đơn:', response.status);
            return [];
        }

        const allInvoices = await response.json();
        const invoices = allInvoices.filter(inv => inv.madatphong == madatphong);
        console.log(' Hóa đơn tìm thấy:', invoices);
...
// async function createServiceInvoiceDetails(mahoadon, services) {
//     const detailResults = [];

//     for (const service of services) {
//         const serviceTotal = (service.giatien || service.giaDichVu || service.price) * service.quantity;

//         // Cấu trúc dữ liệu đầy đủ với madatphong và loaiphi
...
async function confirmServices() {
    ...
    try {
        console.log(' Bắt đầu xử lý đặt dịch vụ cho booking:', currentBookingId);
        console.log(' Dịch vụ được chọn:', selectedServices);

        // Bước 1: Thử tạo hóa đơn mới hoặc sử dụng hóa đơn hiện có
        let mahoadon = null;
        let invoiceResult = null;

        try {
            // Thử tạo hóa đơn mới
            invoiceResult = await createServiceInvoice(currentBookingId, selectedServices);
            mahoadon = invoiceResult.mahoadon || invoiceResult.id;
            console.log(' Hóa đơn mới đã tạo thành công với mã:', mahoadon);

        } catch (invoiceError) {
            console.warn(' Không thể tạo hóa đơn mới:', invoiceError.message);

            // Fallback: Tìm hóa đơn hiện có của booking này
            try {
                const existingInvoices = await getInvoicesByBooking(currentBookingId);
                if (existingInvoices && existingInvoices.length > 0) {
                    // Sử dụng hóa đơn gần nhất
                    const latestInvoice = existingInvoices[existingInvoices.length - 1];
                    mahoadon = latestInvoice.mahoadon || latestInvoice.id;
                    invoiceResult = latestInvoice;
                    console.log(' Sử dụng hóa đơn hiện có:', mahoadon);
                } else {
                    throw new Error('Không tìm thấy hóa đơn để gắn dịch vụ');
                }
            } catch (fallbackError) {
                console.error(' Fallback thất bại:', fallbackError.message);
                throw new Error(`Không thể tạo hoặc tìm hóa đơn: ${invoiceError.message}`);
            }
        }

        if (!mahoadon) {
            throw new Error('Không thể xác định mã hóa đơn');
        } // Bước 2: Lưu vào Sudungdvs (sử dụng dịch vụ hiện có)
        const serviceResults = [];
        for (const service of selectedServices) {
            const serviceData = {
                madatphong: currentBookingId,
                madv: service.madv || service.id,
                soluong: service.quantity
            };

            console.log(' Lưu dịch vụ vào Sudungdvs:', serviceData);

            const serviceResponse = await fetch(`${API_BASE}/Sudungdvs/sudungdv`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (!serviceResponse.ok) {
                ...
                console.warn(` Lỗi lưu dịch vụ ${service.tendv}: ${errorData.message}`);
                // Tiếp tục với các dịch vụ khác
            } else {
                const serviceResult = await serviceResponse.json();
                serviceResults.push(serviceResult);
                console.log(' Đã lưu dịch vụ thành công');
            }
        }

        // // Bước 3: Tạo chi tiết hóa đơn
        // const detailResults = await createServiceInvoiceDetails(mahoadon, selectedServices);

        // Tính toán kết quả cuối
        const totalAmount = selectedServices.reduce((sum, s) => sum + (s.giatien || s.giaDichVu || s.price) * s.quantity, 0);

        const result = {
            success: true,
            message: `Đã tạo hóa đơn và lưu ${selectedServices.length} dịch vụ thành công`,
            data: {
                mahoadon: mahoadon,
                maDatPhong: currentBookingId,
                soLuongDichVu: selectedServices.length,
                tongTienDichVu: totalAmount,
        ...
        console.log('🎉 Hoàn tất tất cả các bước:', result);

        // Hiển thị thông báo thành công với chi tiết
        const isNewInvoice = invoiceResult && (invoiceResult.mahoadon || invoiceResult.id) === mahoadon;
        const invoiceStatus = isNewInvoice ? 'Hóa đơn mới đã được tạo' : 'Đã gắn vào hóa đơn hiện có';

        const successMessage = [
            ' Đặt dịch vụ thành công!',
            '',
            ` Mã hóa đơn: ${mahoadon}`,
            ` Tổng tiền dịch vụ: ${formatCurrency(totalAmount)}`,
            ` Số dịch vụ: ${selectedServices.length}`,
            // ` Chi tiết hóa đơn: ${detailResults.length} mục`,
            ` Trạng thái: ${invoiceStatus}`,
            '',
            'Dịch vụ đã được lưu vào hệ thống!'
        ].join('\n');
...
async function showServiceHistory(bookingId) {
    try {
        ...
        console.log(' Lịch sử dịch vụ (tất cả):', serviceHistory);

        // Lọc chỉ lấy dịch vụ CHƯA thanh toán
        const unpaidServices = serviceHistory.filter(item => {
            const isPaid = item.trangThaiThanhToan === 'Đã thanh toán' || 
                          item.trangthaithanhtoan === 'Đã thanh toán' ||
                          item.isPaid === true ||
                          item.paid === true;
            return !isPaid; // Chỉ lấy những dịch vụ chưa thanh toán
        });

        console.log('💰 Dịch vụ chưa thanh toán:', unpaidServices.length, '/', serviceHistory.length);

        // Clear loading state
        historyBody.innerHTML = '';

        if (unpaidServices && unpaidServices.length > 0) {
            let totalHistoryAmount = 0;
            
            // Try to get customer info from first service record if available
            const firstService = unpaidServices[0];
            ...
            unpaidServices.forEach((item, index) => {
                ...
                row.innerHTML = `
                    <td>${index + 1}</td>
                    <td>${serviceName} <span style="color: #856404; font-size: 0.85em;">⏳ Chưa thanh toán</span></td>
                    <td>${quantity}</td>
            ...
            // Update summary - chỉ hiển thị dịch vụ chưa thanh toán
            document.getElementById('totalServiceCount').textContent = unpaidServices.length;
            ...
            // Show no data state (không có dịch vụ chưa thanh toán)
            historyContent.style.display = 'none';
            noDataDiv.style.display = 'block';
            noDataDiv.innerHTML = `
                <i class="fas fa-check-circle" style="color: #28a745; font-size: 3em; margin-bottom: 10px;"></i>
                <p>Không có dịch vụ chưa thanh toán</p>
                <small style="color: #6c757d;">Tất cả dịch vụ đã được thanh toán</small>
            `;
...
window.createServiceInvoice = createServiceInvoice;
window.getInvoiceDetails = getInvoiceDetails;
window.getInvoicesByBooking = getInvoicesByBooking;
...
function printServiceHistory() {
    try {
        ...
        const printContent = `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Dịch Vụ Chưa Thanh Toán - ${bookingId}</title>
                <style>
                    body {
            ...
            <body>
                <div class="header">
                    <h1>⏳ DỊCH VỤ CHƯA THANH TOÁN</h1>
                    <div class="subtitle">Các dịch vụ đã sử dụng chưa được thanh toán</div>
                </div>
                ...
                <div class="services-section">
                    <h3>⏳ Chi Tiết Dịch Vụ Chưa Thanh Toán</h3>
                    <table class="services-table">
                        <thead>
                            <tr>
                                <th>STT</th>
                ...
                <div class="summary" style="background: #fff3cd; border: 1px solid #ffc107;">
                    <div class="summary-item">
                        <span>Tổng số dịch vụ chưa thanh toán:</span>
                        <span><strong>${totalCount} dịch vụ</strong></span>
                    </div>
                    <div class="summary-item summary-total" style="color: #856404;">
                ...
                <div class="footer">
                    <p>© 2024 Hotel Management System - Danh sách dịch vụ chưa thanh toán được in lúc ${formatDate(new Date())} ${new Date().toLocaleTimeString('vi-VN')}</p>
                    <p style="color: #856404; margin-top: 10px;"><strong>⚠️ Lưu ý: Chỉ hiển thị các dịch vụ chưa được thanh toán</strong></p>
                </div>
            </body>
            </html>
        `;
