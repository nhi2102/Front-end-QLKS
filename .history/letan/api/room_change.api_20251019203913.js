...
async function getRoomTypesAPI() {
    const res = await fetch(`${API_BASE_URL}/LoaiPhong`);
...
async function getAllBookingsAPI() {
    const res = await fetch(`${API_BASE_URL}/DatPhong`);
...
async function getCustomerByIdAPI(customerId) {
    const res = await fetch(`${API_BASE_URL}/KhachHang/${customerId}`);
...
async function getRoomDetailsByBookingIdAPI(bookingId) {
    const res = await fetch(`${API_BASE_URL}/ChiTietDatPhong/booking/${bookingId}`);
...
async function getAvailableRoomsAPI(checkInDate, checkOutDate) {
    const res = await fetch(`${API_BASE_URL}/Phong/available?checkInDate=${checkInDate}&checkOutDate=${checkOutDate}`);
...
async function getRoomByIdAPI(roomId) {
    const res = await fetch(`${API_BASE_URL}/Phong/${roomId}`);
...
async function updateRoomDetailAPI(detailId, updateData) {
    const res = await fetch(`${API_BASE_URL}/ChiTietDatPhong/${detailId}`, {
        method: 'PUT',
...
async function updateBookingAPI(bookingId, bookingData) {
    const res = await fetch(`${API_BASE_URL}/DatPhong/${bookingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
