...
async function loadBookings() {
    try {
        showLoading();
        const data = await fetchBookingsData(); // Gọi thật từ backend
        allBookings = data;
...
async function fetchBookingsData() {
    ...
    return data.map(item => ({
        ...
        nights: item.trangThaiThanhToan,
        guests: item.soNguoi || 1,
        roomPrice: item.tongTien / (item.trangThaiThanhToan || 1),
...
function getActionButtons(booking) {
    ...
    if (booking.status === 'Đã đặt' && checkinDate.getTime() === today.getTime()) {
    ...
    if (booking.status === 'Chờ xác nhận' || booking.status === 'Đã đặt') {
...
function updateStatistics(bookings) {
    ...
    bookings.forEach(booking => {
        ...
        // Upcoming
        if (checkin > today && (booking.status === 'Đã đặt' || booking.status === 'Đã đặt')) {
            stats.upcoming++;
        }
...
function viewBookingDetail(bookingId) {
    ...
    document.getElementById('modalNights').textContent = selectedBooking.nights;
    ...
    document.getElementById('btnEdit').style.display =
        (selectedBooking.status === 'Đã đặt' || selectedBooking.status === 'Đã đặt') ? 'flex' : 'none';
    document.getElementById('btnCancel').style.display =
        (selectedBooking.status === 'Đã đặt' || selectedBooking.status === 'Đã đặt' || selectedBooking.status === 'Đang ở') ? 'flex' : 'none';
