...
// Tài khoản người dùng - kiểm tra đăng nhập
...
async function fetchAvailableRooms(checkInDate, checkOutDate, guestCount, roomType) {
    try {
        ...
        console.log(` Rooms after details fetch: ${validRooms.length} (requested ${roomsData.length})`);

        if (validRooms.length === 0) {
            console.log('No valid room details could be fetched. Returning empty list.');
            return [];
        }
        ...
        console.log('Rooms after enrichment:', enrichedRooms.length);
