...
async function promptAndResetPassword() {
    // Lấy ID và Tên khách hàng từ form edit
    const customerId = document.getElementById('editCustomerId').value;
    ...
    if (!customerId) {
        alert('Không thể xác định khách hàng để reset mật khẩu.');
        return;
    }

    if (!customerEmail) {
        alert('Khách hàng này không có email. Không thể gửi link reset.');
        return;
    }

    // Yêu cầu Lễ tân xác nhận
    const isConfirmed = confirm(
        `Bạn có chắc chắn muốn gửi link reset mật khẩu cho khách hàng "${customerName}"?\n\n` +
        `Link sẽ được gửi đến email đã đăng ký: ${customerEmail}`
    );

    if (isConfirmed) {
        // Gọi hàm API để gửi
        await sendPasswordResetLink(customerId, customerEmail);
    }
}
// ============================================
// UTILITY FUNCTIONS
