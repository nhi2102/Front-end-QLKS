...
// checkout.api.jslet currentBooking = null;

// ============================let originalRoomCharge = 0;



const API_BASE_URL = "https://localhost:7076/api";// Initialize page

document.addEventListener('DOMContentLoaded', function() {

// --- Helper: format tiền ---    checkUserLogin();

function formatCurrency(amount) {    loadCurrentGuests();

    if (!amount || amount === 0) return "0 ₫";    setupEventListeners();

    return new Intl.NumberFormat('vi-VN', {    setDefaultCheckOutTime();

        style: 'currency',});

        currency: 'VND'

    }).format(amount);// Check user login
...
function formatDate(dateString) {    if (!currentUser) {

    if (!dateString) return '-';        alert("Bạn cần đăng nhập để truy cập trang này!");
...
// API functions        document.getElementById("userName").textContent = "Nhân Viên";
...
async function fetchPendingCheckouts() {// Helper function to safely set text content

    const res = await fetch(`${API_BASE_URL}/Datphongs/pending-checkouts`);function safeSetText(elementId, value) {

    if (!res.ok) throw new Error(`HTTP ${res.status}`);    const element = document.getElementById(elementId);

    return res.json();    if (element) {

}        element.textContent = value || '-';

    } else {

// --- Lấy lịch sử dịch vụ (chỉ dịch vụ chưa thanh toán) ---        console.warn(` Element not found: ${elementId}`);
...
async function fetchServiceHistory(bookingId) {    }
    ...
    return res.json();// Format currency

}function formatCurrency(amount) {

    if (!amount || amount === 0) return "0 ₫";

// --- Lấy danh sách thiết bị khách sạn ---    return new Intl.NumberFormat('vi-VN', {

async function fetchHotelEquipment() {        style: 'currency',

    const res = await fetch(`${API_BASE_URL}/Thietbikhachsans`);        currency: 'VND'

    if (!res.ok) throw new Error(`HTTP ${res.status}`);    }).format(amount);
...
// --- Lấy danh sách đền bù thiệt hại theo booking ---function setupEventListeners() {

async function fetchDamageCompensation(bookingId) {    const menuToggle = document.getElementById('menuToggle');

    const res = await fetch(`${API_BASE_URL}/Denbuthiethais`);    if (menuToggle) {

    if (!res.ok) throw new Error(`HTTP ${res.status}`);        menuToggle.addEventListener('click', toggleSidebar);
...
async function createDamageCompensation(data) {    // Extra charge manual change handler

    const res = await fetch(`${API_BASE_URL}/Denbuthiethais/taodenbu`, {    document.addEventListener('input', function(event) {

        method: 'POST',        if (event.target.id === 'extraCharge') {

        headers: { 'Content-Type': 'application/json' },            // Update base amount when user manually changes extra charge

        body: JSON.stringify(data)            const extraChargeEl = document.getElementById('extraCharge');

    });            if (extraChargeEl) {

    if (!res.ok) throw new Error(`HTTP ${res.status}`);                extraChargeEl.dataset.baseAmount = extraChargeEl.value;
...
async function getRemainingAmount(bookingId) {
    ...
    const res = await fetch(`${API_BASE_URL}/Datphongs/Sotienthieu/${bookingId}`);function toggleSidebar() {

    if (!res.ok) throw new Error('Lỗi khi lấy số tiền còn thiếu');    const sidebar = document.querySelector('.sidebar');

    return res.json();    if (sidebar) {
...
async function executeCheckout(bookingId) {
    ...
    const res = await fetch(`${API_BASE_URL}/Datphongs/Checkout/${bookingId}`);// Set default checkout time

    if (!res.ok) throw new Error('Lỗi khi thực hiện checkout');function setDefaultCheckOutTime() {

    return res.json();    const now = new Date();

}    const hours = String(now.getHours()).padStart(2, '0');

    const minutes = String(now.getMinutes()).padStart(2, '0');

// --- Tính tổng tiền dịch vụ chưa thanh toán ---    const currentTime = `${hours}:${minutes}`;

async function calculateTotalServiceAmount(bookingId) {

    const history = await fetchServiceHistory(bookingId);    const checkOutTimeEl = document.getElementById('checkOutTime');

    return history.reduce((sum, s) => sum + (s.thanhTien || 0), 0);    if (checkOutTimeEl) {
...
function computeTotal(serviceCharge, extra, discount) {
    ...
    const total = (serviceCharge || 0) + (extra || 0) - (discount || 0);// Check if date is today

    return total > 0 ? total : 0;function isToday(dateString) {

}    if (!dateString) return false;

//Lấy thông tin khách hàng (khi cần)

async function getCustomerById(customerId) {    try {

    const response = await fetch(`${API_BASE}/Khachhangs/${customerId}`);        const date = new Date(dateString);

    if (!response.ok) throw new Error('Lỗi tải dữ liệu khách hàng');        const today = new Date();
...
//Lấy chi tiết đặt phòng (nếu cần)            date.getDate() === today.getDate();

async function getRoomDetailsByBookingId(bookingId) {    } catch (error) {

    const response = await fetch(`${API_BASE}/Chitietdatphongs?madatphong=${bookingId}`);        return false;
...
// Lấy thông tin phòng theo mãfunction formatDate(dateString) {

async function getRoomById(roomId) {    if (!dateString) return '-';

    const response = await fetch(`${API_BASE}/Phongs/${roomId}`);

    if (!response.ok) throw new Error('Lỗi tải thông tin phòng');    try {

    return response.json();        const date = new Date(dateString);

}        return date.toLocaleDateString('vi-VN', {

// Gắn các hàm vào global để UI có thể gọi            day: '2-digit',

window.formatCurrency = formatCurrency;            month: '2-digit',

window.formatDate = formatDate;            year: 'numeric'

window.fetchPendingCheckouts = fetchPendingCheckouts;        });

window.fetchServiceHistory = fetchServiceHistory;    } catch (error) {

window.fetchHotelEquipment = fetchHotelEquipment;        return dateString;
...
window.executeCheckout = executeCheckout;// Load current guests (pending checkouts for today)

window.calculateTotalServiceAmount = calculateTotalServiceAmount;async function loadCurrentGuests() {

window.computeTotal = computeTotal;    try {

window.getCustomerById = getCustomerById;        console.log(' Loading pending checkouts for today...');

window.getRoomDetailsByBookingId = getRoomDetailsByBookingId;

window.getRoomById = getRoomById;        const response = await fetch(`${API_BASE_URL}/Datphongs/pending-checkouts`);
        if (!response.ok) {
    ...
    try {
        ...
        for (const checkbox of equipmentItems) {
            ...
            const damageData = {
                ...
                thanhTien: equipmentPrice,
                lyDo: `Thiết bị ${equipmentName} bị hư hỏng khi checkout`,
            ...
            try {
                ...
                if (response.ok) {
                    console.log(' Equipment damage recorded:', equipmentName);
                } else {
