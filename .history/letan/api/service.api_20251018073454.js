...
async function loadAvailableServices() {
    try {
        // If navigation preloader already stored services and they're fresh (2 minutes), use them
        const ts = sessionStorage.getItem('preload_timestamp');
        const cached = sessionStorage.getItem('preloaded_services');
        if (cached && ts) {
            const age = Date.now() - Number(ts || 0);
            const isFresh = !isNaN(age) && age < 2 * 60 * 1000; // 2 minutes
            if (isFresh) {
                try {
                    availableServices = JSON.parse(cached);
                    return availableServices;
                } catch (e) {
                    console.debug('Failed to parse cached preloaded_services, will fetch', e);
                }
            }
        }
        ...
        // store a cached copy for other pages / future calls
        try {
            sessionStorage.setItem('preloaded_services', JSON.stringify(availableServices));
            sessionStorage.setItem('preload_timestamp', Date.now().toString());
        } catch (e) {
            // ignore storage errors (e.g., quota), but log for diagnostics
            console.debug('Could not write preloaded_services to sessionStorage', e);
        }

        return availableServices;
    } catch (err) {
        console.warn('loadAvailableServices failed', err);
        throw err;
    }
}
