...
async function loadCustomers() {
    ...
    try {
        ...
        await Promise.all(allCustomers.map(async(customer) => {
            ...
            // Tính điểm đã sử dụng từ hóa đơn
            customer.diemdadung = await getCustomerUsedPoints(customer.makh);
        }));
...
async function getCustomerUsedPoints(customerId) {
    try {
        ...
        for (const booking of customerBookings) {
            try {
                ...
                // Tính tổng điểm đã sử dụng (loại phí = "Điểm")
                for (const detail of billDetails) {
                    if (detail.loaiphi && detail.loaiphi.toLowerCase() === 'điểm') {
                        // Giá trị âm của số tiền = số điểm đã dùng
                        totalUsedPoints += Math.abs(detail.sotien || 0);
                    }
                }
            } catch (error) {
                console.error(`Lỗi xử lý hóa đơn cho đặt phòng ${booking.madatphong}:`, error);
            }
        }
        
        return totalUsedPoints;
    } catch (error) {
        console.error('Lỗi tính điểm đã sử dụng:', error);
        return 0;
    }
}
...
window.viewCustomerDetail = async function(customerId) {
    ...
    // Cập nhật lại điểm đã sử dụng (để có dữ liệu mới nhất)
    showLoading(true);
    customer.diemdadung = await getCustomerUsedPoints(customer.makh);
    showLoading(false);
...
function displayCustomerInfo(customer) {
    ...
    infoGrid.innerHTML = `
        ...
        <div class="info-item">
            <div class="label">Điểm Thành Viên</div>
            <div class="value">${customer.diemthanhvien || 0}</div>
        </div>
        <div class="info-item">
            <div class="label">Điểm Đã Sử Dụng</div>
            <div class="value">${customer.diemdadung || 0}</div>
        </div>
    `;
}
