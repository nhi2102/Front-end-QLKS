...
async function handleEquipmentDamageOnCheckout(bookingId) {
    try {
        ...
        for (const checkbox of equipmentItems) {
            ...
            // Get selected room
            const roomSelect = document.getElementById(`room_${equipmentId}`);
            const selectedRoomNumber = roomSelect ? roomSelect.value : '';
            
            // Get quantity
            const quantityInput = document.getElementById(`quantity_${equipmentId}`);
            const quantity = quantityInput ? (parseInt(quantityInput.value) || 1) : 1;
            
            // Get MAPHONG from selected room number
            let maPhong = null;
            if (currentBooking && currentBooking.chitietdatphongs) {
                const chitiet = currentBooking.chitietdatphongs.find(ct => {
                    const sophong = ct.phong?.sophong || ct.phong?.soPhong || ct.sophong || ct.soPhong;
                    return sophong == selectedRoomNumber;
                });
                if (chitiet) {
                    maPhong = chitiet.maphong || chitiet.maPhong || chitiet.phong?.maphong || chitiet.phong?.maPhong;
                }
            }
            
            // If not found in chitietdatphongs, try danhSachPhong
            if (!maPhong && currentBooking && currentBooking.danhSachPhong) {
                const room = currentBooking.danhSachPhong.find(r => {
                    const sophong = r.sophong || r.soPhong;
                    return sophong == selectedRoomNumber;
                });
                if (room) {
            ...
            console.log(`   - Room Number: ${selectedRoomNumber}, Room ID: ${maPhong}`);
            console.log(`   - Quantity: ${quantity}`);

            // Call stored procedure TAO_DENBUTHIETHAI
            const procedureData = {
                maThietBi: parseInt(equipmentId),
                maDatPhong: parseInt(bookingId),
                soLuong: parseInt(quantity),
                maPhong: parseInt(maPhong)
            };

            console.log('📤 Calling TAO_DENBUTHIETHAI with params:', procedureData);

            // Validate maPhong
            if (!maPhong) {
                console.error('❌ Cannot find MAPHONG for room:', selectedRoomNumber);
                alert(`Không tìm thấy mã phòng cho phòng ${selectedRoomNumber}`);
                continue;
            }

            try {
                // Try using stored procedure endpoint first
                let response = await fetch(`${API_BASE_URL}/Denbuthiethais/execute-procedure`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.log(`✅ Equipment damage recorded: ${equipmentName} - Room ${selectedRoomNumber} (ID: ${maPhong}) - Qty ${quantity}`);
                } else if (response.status === 404) {
                    // Endpoint not found, try alternative URL pattern
                    console.warn('⚠️ Endpoint not found, trying alternative: /Denbuthiethais/CreateWithProcedure');
                    
                    response = await fetch(`${API_BASE_URL}/Denbuthiethais/CreateWithProcedure`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (response.ok) {
                        console.log(`✅ Equipment damage recorded via alternative endpoint`);
                    } else {
                        const errorText = await response.text();
                        console.error('❌ Failed with alternative endpoint:', errorText);
                    }
                } else {
                    const errorText = await response.text();
