...
function displayServiceHistoryInModal(serviceHistory) {
    ...
    const historyHTML = serviceHistory.map((service, index) => {
        ...
        const isPaid = service.trangThaiThanhToan === 'Đã thanh toán' || 
                      service.trangthaithanhtoan === 'Đã thanh toán' ||
                      service.isPaid === true ||
                      service.paid === true;
        ...
        console.log(` Service ${index + 1}: ${serviceName} - Qty: ${quantity}, Unit: ${unitPrice}, Total: ${totalPrice}, Paid: ${isPaid}`);

        totalServiceAmount += totalPrice;
        if (!isPaid) {
            totalUnpaidAmount += totalPrice;
        }

        return `
            <div class="service-history-item ${isPaid ? 'paid-service' : 'unpaid-service'}">
                <div class="service-info">
                    <div class="service-name">${serviceName} ${paymentBadge}</div>
    ...
    serviceHistoryContainer.innerHTML = `
        ...
        <div class="service-history-total">
            <div class="total-row">
                ...
                <strong>${formatCurrency(totalServiceAmount)}</strong>
            </div>
            <div class="total-row unpaid-highlight">
    ...
    console.log('📊 Displayed', serviceHistory.length, 'services | Total:', totalServiceAmount, '| Unpaid:', totalUnpaidAmount);
}
...
function openCheckOutModal(booking) {
    ...
    fetchAndDisplayServiceHistory(bookingId).then(serviceHistory => {
        // Calculate total service amount from unpaid services only
        let totalServiceAmount = 0;
        serviceHistory.forEach(service => {
            // Check payment status - only count unpaid services
            const isPaid = service.trangThaiThanhToan === 'Đã thanh toán' || 
                          service.trangthaithanhtoan === 'Đã thanh toán' ||
                          service.isPaid === true ||
                          service.paid === true;
        ...
        // Update service charge display with the calculated amount (unpaid only)
        if (serviceChargeEl) {
            serviceChargeEl.textContent = formatCurrency(totalServiceAmount);
        }

        console.log('💰 Unpaid service charge:', totalServiceAmount);
        calculateTotal();
    }).catch(error => {
