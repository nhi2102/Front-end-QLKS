// ============================================
// CUSTOMER MANAGEMENT PAGE - API & LOGIC
// ============================================
...
async function loadCustomers() {
    ...
    try {
        ...
        // Load booking data for each customer
...
function applyFilters() {
    ...
    filteredCustomers = allCustomers.filter(customer => {
        ...
        // Gender filter
        if (gender && customer.gioitinh !== gender) return false;
...
function displayCustomerInfo(customer) {
    ...
    infoGrid.innerHTML = `
        ...
        <div class="info-item">
            ...
            <div class="value">${customer.gioitinh || 'N/A'}</div>
        </div>
        <div class="info-item">
        ...
        <div class="info-item" style="grid-column: 1 / -1;">
            ...
            <div class="value">${customer.ghichu || 'Không có'}</div>
        </div>
    `;
}
...
async function createBookingHistoryItem(booking) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/ChiTietDatPhong/booking/${booking.madatphong}`);
        if (response.ok) {
...
function displayCustomerStats(customer) {
    ...
    const completedBookings = customer.bookings ? .filter(b =>
        b.trangthai === 'Đã thanh toán' || b.trangthai === 'Đã trả phòng'
    ).length || 0;
    const cancelledBookings = customer.bookings ? .filter(b => b.trangthai === 'Đã hủy').length || 0;
    const totalSpent = customer.bookings ? .reduce((sum, b) => sum + (b.tongcong || 0), 0) || 0;
...
window.editCustomer = function(customerId) {
    ...
    document.getElementById('editGioitinh').value = customer.gioitinh || '';
    ...
    document.getElementById('editGhichu').value = customer.ghichu || '';
...
async function saveCustomer() {
    ...
    try {
        ...
        if (isEdit) {
            ...
            response = await fetch(`${API_BASE_URL}/KhachHang/${customerId}`, {
                method: 'PUT',
                headers: {
            ...
            // Create new customer
            response = await fetch(`${API_BASE_URL}/KhachHang`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
