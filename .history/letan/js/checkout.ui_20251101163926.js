...
async function openCheckOutModal(booking) {
    ...
    // Xử lý dữ liệu từ API - TÍNH SỐ LƯỢNG DỊCH VỤ CHƯA THANH TOÁN
    if (services) {
        // Trường hợp API trả về object có field chuaThanhToan
        if (services.chuaThanhToan && Array.isArray(services.chuaThanhToan)) {
            pendingServices = services.chuaThanhToan;
            totalService = services.tongChuaThanhToan || 0;
        }
        // Trường hợp API trả về dữ liệu tổng hợp cần tính toán
        else if (services.sudungdv && services.dathanhToan) {
            // Có cả dịch vụ đã dùng và đã thanh toán
            const usedServices = services.sudungdv || [];
            const paidServices = services.dathanhToan || [];
            
            // Tính số lượng còn phải thanh toán cho từng dịch vụ
            pendingServices = usedServices.map(used => {
                const paid = paidServices.find(p => p.madv === used.madv || p.maDv === used.madv) || {};
                const soLuongDaDung = used.soluong || used.soLuong || 0;
                const soLuongDaThanhToan = paid.soluong || paid.soLuong || 0;
                const soLuongConLai = soLuongDaDung - soLuongDaThanhToan;
                
                if (soLuongConLai > 0) {
                    const donGia = used.dongia || used.donGia || 0;
                    return {
                        tenDichVu: used.tendichvu || used.tenDichVu,
            ...
            totalService = pendingServices.reduce((sum, s) => sum + s.thanhTien, 0);
        }
        // Trường hợp API trả về mảng trực tiếp
        else if (Array.isArray(services)) {
            pendingServices = services.filter(s => s.trangThaiThanhToan === 'Chưa thanh toán' || 
                                                   s.TrangThaiThanhToan === 'Chưa thanh toán' ||
                                                   !s.trangThaiThanhToan);
            totalService = pendingServices.reduce((sum, s) => sum + (s.thanhTien || s.ThanhTien || 0), 0);
        }
    }

    console.log('💰 Pending services:', pendingServices);
    console.log('💰 Total service charge:', totalService);

    document.getElementById('serviceCharge').textContent = formatCurrency(totalService);
    
    // Hiển thị CHỈ các dịch vụ chưa thanh toán (số lượng còn lại)
    document.getElementById('serviceHistoryList').innerHTML =
        pendingServices.length ?
        pendingServices.map(s => `
            <div class="service-item" style="display: flex; justify-content: space-between; padding: 5px 0;">
                <span>
                    ${s.tenDichVu || s.TenDichVu} 
                    <small style="color: #666;">(SL: ${s.soLuong || s.SoLuong || 1})</small>
                </span>
                <span style="font-weight: bold;">
                    ${formatCurrency(s.thanhTien || s.ThanhTien || 0)}
                    <small style="color: #dc3545; margin-left: 8px;">(Chưa TT)</small>
                </span>
            </div>
        `).join('') :
        '<div style="color: #28a745; text-align: center; padding: 10px;">✅ Không có dịch vụ chưa thanh toán</div>';
...
async function printCheckoutInvoice(booking) {
    ...
    try {
        ...
        // Lấy CHỈ dịch vụ chưa thanh toán để in hóa đơn
        const services = await CheckoutAPI.getServiceHistory(booking.maDatPhong);
        let pendingServices = [];
        ...
        console.log('📄 Printing services (unpaid only):', pendingServices);
