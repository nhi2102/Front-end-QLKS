...
function setupChangeTypeToggle() {
    ...
    if (radios.length === 0) {
        console.warn("⚠️ Không tìm thấy radio 'changeType' trong DOM!");
        return;
    }
        ...
        radio.addEventListener('change', async() => {
            if (radio.checked) {
                if (radio.value === 'changeRoom') {
                    console.log("🟢 Đã chọn: Đổi phòng cùng ngày");
                    ...
                    if (currentBooking) {
                        ...
                        alert('Vui lòng chọn một đơn đặt phòng trước!');
                        radio.checked = false;
                    }
                } else if (radio.value === 'changeDate') {
                    console.log("🟠 Đã chọn: Đổi ngày đặt phòng");
...
async function selectBooking(booking) {
    ...
    try {
        ...
        // Chỉ hiện form đổi phòng, KHÔNG load phòng trống ngay
        const section = document.getElementById('roomChangeFormSection');
        if (section) section.style.display = 'block';

        // Ẩn luôn hai phần con cho tới khi chọn radio
