...
async function selectBooking(booking) {
    ...
    try {
        ...
        // Kiểm tra trạng thái — chỉ "Đã đặt" mới được đổi ngày
        const changeDateRadio = document.querySelector('input[value="changeDate"]');
        const notice = document.getElementById('statusNotice');

        if (booking.trangthai === "Đang ở") {
            changeDateRadio.disabled = true;
            if (notice) {
                notice.style.display = "block";
                notice.textContent = "⚠️ Khách đang ở — chỉ được phép đổi phòng, không thể đổi ngày.";
            }
        } else if (booking.trangthai === "Đã đặt") {
            changeDateRadio.disabled = false;
