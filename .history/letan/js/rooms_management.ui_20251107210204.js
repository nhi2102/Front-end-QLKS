...
async function updateRoomStatus() {
    ...
    // Kiểm tra nếu phòng đang có khách
    if (selectedRoom.tenKhach && (newStatus === 'Bảo trì' || newStatus === 'Đang dọn dẹp')) {
        const statusText = newStatus === 'Bảo trì' ? 'bảo trì' : 'dọn dẹp';
        if (!confirm(`⚠️ Phòng hiện đang có khách (${selectedRoom.tenKhach}).\n\nBạn có chắc chắn muốn chuyển sang trạng thái ${statusText}?\n\nLưu ý: Hệ thống sẽ kiểm tra và từ chối nếu khách vẫn còn trong thời gian lưu trú.`)) {
            return;
        }
    }
    ...
    try {
        showLoading();
        const result = await RoomAPI.updateRoomStatus(selectedRoom.maPhong, newStatus);
        ...
        // Hiển thị thông báo lỗi cụ thể từ stored procedure
        let errorMessage = err.message || 'Không thể cập nhật trạng thái phòng.';
        
        // Xử lý các lỗi thường gặp với thông báo thân thiện
        if (errorMessage.includes('Không tìm thấy phòng')) {
            errorMessage = '❌ Không tìm thấy phòng này trong hệ thống!';
        } else if (errorMessage.includes('trùng với trạng thái hiện tại')) {
            errorMessage = 'ℹ️ Trạng thái bạn chọn giống với trạng thái hiện tại của phòng!';
        } else if (errorMessage.includes('đang có khách ở')) {
            errorMessage = '⚠️ Phòng đang có khách lưu trú, không thể thay đổi trạng thái!\n\nVui lòng đợi khách check-out hoặc liên hệ quản lý.';
        } else if (errorMessage.includes('bảo trì sang trạng thái đang dọn dẹp')) {
            errorMessage = '⚠️ Phòng đang bảo trì không thể chuyển sang trạng thái dọn dẹp!\n\nVui lòng chuyển sang "Hoạt động" trước.';
        }
...
function openRoomDetail(id) {
    ...
    // Set giá trị mặc định cho dropdown trạng thái mới
    const statusSelect = document.getElementById('newStatus');
    if (statusSelect) {
        // Map trạng thái thực tế sang trạng thái trong DB
        const statusMap = {
            'Trống': 'Hoạt động',
        ...
        const mappedStatus = statusMap[selectedRoom.trangThaiThucTe] || 'Hoạt động';
...
// Hàm hiển thị notification modal
