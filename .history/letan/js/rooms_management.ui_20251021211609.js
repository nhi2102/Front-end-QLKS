...
function displayGridView(rooms) {
    ...
    container.innerHTML = rooms.map(r => `
        <div class="room-card" onclick="openRoomDetail(${r.maphong})">
            ...
            <div class="room-type"><i class="fas fa-bed"></i> ${r.maloaiphongNavigation?.tenloaiphong || 'N/A'}</div>
            <div class="room-details">
                <i class="fas fa-building"></i> Tầng ${r.tang || '-'}<br>
...
function applyFilters() {
    ...
    if (type) filtered = filtered.filter(r => r.maloaiphongNavigation ? .tenloaiphong === type);
...
function openRoomDetail(id) {
    ...
    document.getElementById('modalRoomType').textContent = selectedRoom.maloaiphongNavigation ? .tenloaiphong || '-';
