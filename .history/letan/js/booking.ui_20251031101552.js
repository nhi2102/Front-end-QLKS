...
function displayAvailableRooms(rooms) {
    ...
    grid.innerHTML = rooms.map(room => {
                ...
                const roomType =
                    ...
                    room.tenloaiphong || room.maloaiphong || 'Không rõ loại';
                const capacity =
                    room.soLuongNguoi || room.succhua ||
                    (room.loaiPhong &&
                        (room.loaiPhong.songuoitoida ||
                            room.loaiPhong.soLuongNguoi ||
                            room.loaiPhong.succhua)) ||
                    'N/A';
                ...
                const selectBtn = `<button class="select-room-btn ${selected ? 'selected-btn' : ''}" onclick="selectRoom('${roomId}')">
...
function displaySelectedRoomInfo() {
    ...
    if (!selectedRooms.length) {
        ...
        // Clear summary as well
        updateRoomSummaryInModal();
        return;
    }

    el.innerHTML = selectedRooms.map((r, index) => {
        const roomType = r.loaiPhong?.tenloaiphong || 'Không rõ loại';
        const roomPrice = r.loaiPhong?.giacoban || 0;
        const roomNumber = r.sophong || r.soPhong || 'N/A';
        const capacity = r.loaiPhong?.songuoitoida || r.succhua || 'N/A';
...
function removeSelectedRoom(roomId) {
    ...
    updateBookingSummary();
    displayAvailableRooms(availableRooms);
    updateRoomSummaryInModal(); // Update modal summary
}
...
function updateBookingSummary() {
    ...
    // Tổng giá phòng mỗi đêm
    const totalPerNight = selectedRooms.reduce((sum, r) => sum + (r.loaiPhong?.giacoban || 0), 0);
...
async function submitBooking(e) {
    ...
    try {
       ...
       const details = selectedRooms.map(r => ({
            Maphong: r.maphong,
            Tongcong: r.loaiPhong?.giacoban || 0,
...
function openBookingModal() {
    ...
    if (modal) {
        ...
        // Update selected room summary in modal
        updateRoomSummaryInModal();
    }
}
...
function updateRoomSummaryInModal() {
    const summaryContainer = document.getElementById('selectedRoomSummary');
    if (!summaryContainer || selectedRooms.length === 0) return;

    let summaryHTML = '<h4><i class="fas fa-bed"></i> Tóm tắt phòng đã chọn:</h4>';
    let totalPrice = 0;

    selectedRooms.forEach(room => {
        const roomTotal = room.price * room.nights;
        totalPrice += roomTotal;
        
        summaryHTML += `
            <div class="room-summary-item">
                <div class="room-name">
                    ${room.name} (${room.nights} đêm)
                </div>
                <div class="room-price">
    ...
    summaryHTML += `
        <div class="room-summary-item" style="font-weight: 600; border-top: 2px solid #0d6efd; padding-top: 10px;">
            <div class="room-name">Tổng cộng:</div>
            <div class="room-price" style="color: #dc3545; font-size: 1.1rem;">
...
function submitBookingFromModal() {
    // Get the form inside the modal and submit it
...
// Update the existing selectRoom function to update summary
const originalSelectRoom = window.selectRoom;
if (originalSelectRoom) {
    window.selectRoom = function(roomId, button) {
        originalSelectRoom(roomId, button);
        updateRoomSummaryInModal();
    };
}
