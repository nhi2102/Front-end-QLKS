...
async function loadAvailableRooms(booking) {
    // Nếu lễ tân chọn ngày thủ công thì dùng, nếu không thì dùng ngày đơn đặt
    const checkInInput = document.getElementById('changeFromDate').value;
    const checkOutInput = document.getElementById('changeToDate').value;

    const checkIn = checkInInput || booking.ngaynhanphong.split('T')[0];
    const checkOut = checkOutInput || booking.ngaytraphong.split('T')[0];

    console.log(`[ROOM CHANGE] Kiểm tra phòng trống từ ${checkIn} → ${checkOut}`);
