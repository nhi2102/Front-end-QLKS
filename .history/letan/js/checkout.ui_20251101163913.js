...
async function openCheckOutModal(booking) {
    ...
    // Hiển thị CHỈ các dịch vụ chưa thanh toán
    document.getElementById('serviceHistoryList').innerHTML =
        pendingServices.length ?
        pendingServices.map(s => `
            <div class="service-item">
                <span>${s.tenDichVu || s.TenDichVu}</span>
                <span>${formatCurrency(s.thanhTien || s.ThanhTien || 0)}</span>
                <small style="color: #dc3545;">(Chưa thanh toán)</small>
            </div>
        `).join('') :
        '<div style="color: #28a745;">✅ Không có dịch vụ chưa thanh toán</div>';
