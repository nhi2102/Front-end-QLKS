...
async function searchBookings() {
    ...
    try {
        ...
        switch (searchType) {
            ...
            case 'room':
                for (const b of bookings) {
                    const details = await RoomChangeAPI.getRoomDetailsByBookingIdAPI(b.madatphong);
                    const phong = details && details[0] && details[0].phong;
                    if (phong && phong.sophong.toString().includes(keyword)) {
                        filtered.push({
                            ...b,
                            sophong: phong.sophong,
                            tenloaiphong: phong.loaiPhong && phong.loaiPhong.tenloaiphong
                        });
                    }
                }
                break;
...
function displayCurrentBookingInfo(booking, details, customer) {
    ...
    info.innerHTML = `
        <p><b>Khách:</b> ${customer.tenKhachHang}</p>
