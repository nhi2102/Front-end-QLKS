...
function displayAvailableRooms(rooms) {
    ...
    grid.innerHTML = rooms.map(room => {
                ...
                const roomType = loaiPhong.tenloaiphong || loaiPhong.tenLoaiPhong || 'Không rõ loại';
                const capacity = room.succhua || loaiPhong.succhua || 'N/A';

                // === GIÁ ===
                const giaCoBan = loaiPhong.giacoban || loaiPhong.giaCoBan || 0;
                const giaKhuyenMai = loaiPhong.giaKhuyenMai || loaiPhong.giakhuyenmai || null;
                const tenVoucher = loaiPhong.tenVoucher || loaiPhong.tenvoucher || null;
        ...
        `;
                ...
                // === HIỂN THỊ GIÁ (ghi đè nếu có voucher) ===
                let priceHtml = `
            <div class="price-info">
                <span class="price-amount">${formatCurrency(giaCoBan)}</span>
        ...
        `;
                ...
                <div class="price-info discount">
                    <span class="price-original">${formatCurrency(giaCoBan)}</span>
                    <span class="price-discount">${formatCurrency(giaKhuyenMai)}</span>
                    <span class="price-unit">/ đêm</span>
                </div>
                ${tenVoucher ? `<div class="voucher-tag"><i class="fas fa-tag"></i> ${tenVoucher}</div>` : ''}
            `;
        }
