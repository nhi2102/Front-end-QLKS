...
function showProfileModal() {
    ...
    // Hiển thị ngày sinh
    const birthdate = currentUser.ngaysinh || currentUser.birthdate;
    if (birthdate) {
        const date = new Date(birthdate);
        document.getElementById('profile_birthdate').textContent = date.toLocaleDateString('vi-VN');
    } else {
        document.getElementById('profile_birthdate').textContent = '-';
    }
    
    // Hiển thị địa chỉ
    document.getElementById('profile_address').textContent = currentUser.diachi || currentUser.address || '-';
