...
async function showServiceHistory(bookingId) {
    ...
    // load history defensively
    let data;
    try {
        console.debug('showServiceHistory: fetching history for', bookingId);
        data = await api.getServiceHistory(bookingId);
        console.debug('showServiceHistory: raw data', data);
    } catch (err) {
        console.error('showServiceHistory: failed to load history', err);
        if (body) body.innerHTML = `<tr><td colspan="6">Lỗi khi tải lịch sử dịch vụ: ${err.message || err}</td></tr>`;
    ...
    if (!Array.isArray(data) || data.length === 0) {
        if (body) body.innerHTML = '<tr><td colspan="6">Chưa có dịch vụ nào</td></tr>';
        const noHistory = document.getElementById('noHistoryData');
    ...
    body.innerHTML = data.map((item, i) => {
        ...
        const usedDate = api.formatDate(candidateDate);
        return `
        <tr>
            <td>${i + 1}</td>
