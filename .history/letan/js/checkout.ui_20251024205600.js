...
function handleEquipmentSelectionChange() {
    ...
    items.forEach(item => {
        ...
        if (checkbox.checked) {
            //  Hiện phần chọn phòng và số lượng
    ...
    //  Cập nhật lại tổng tiền bồi thường
    let total = 0;
    ...
    // Gán tổng vào phần hiển thị
    const compEl = document.getElementById('equipmentCompensation');
    if (compEl) compEl.textContent = formatCurrency(total);
}
...
async function handleCheckout() {
    const bookingId = currentBooking ? .maDatPhong;
    if (!bookingId) {
    ...
    try {
        // 1️⃣ Lấy danh sách thiết bị hư hỏng từ giao diện
        const damagedItems = [];
        document.querySelectorAll('.equipment-check:checked').forEach(cb => {
