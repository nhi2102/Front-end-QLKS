...
function debugBookingData() {
    ...
    console.log('🔍 DEBUG - Tổng số booking:', allBookings.length);
    ...
    console.log('📊 Thống kê trạng thái:', statusCount);
    
    // Check-in hôm nay
    const checkinToday = allBookings.filter(b => 
        new Date(b.checkinDate).toDateString() === todayStr
    );
    console.log(`🏨 Check-in hôm nay (tất cả): ${checkinToday.length}`);
    console.log('   - Trạng thái:', checkinToday.map(b => `${b.id}: ${b.status}`));
...
function filterBookings(filter) {
    ...
    switch (filter) {
        ...
        case 'checkin-today':
            ...
            console.log(`🏨 Check-in hôm nay (Đã đặt): ${filteredBookings.length} đặt phòng`);
            break;
        case 'checkout-today':
            ...
            console.log(`🚪 Check-out hôm nay (Đang ở): ${filteredBookings.length} đặt phòng`);
            break;
        case 'upcoming':
...
function getActionButtons(b) {
    ...
    if (b.status === 'Đang ở') {
        console.log(`🚫 Booking ${b.id} (${b.status}) - Không hiển thị nút hủy`);
    }
...
function viewBookingDetail(id) {
    ...
    if (btnCancel) {
        ...
        console.log(`🔍 Modal Detail - Booking ${selectedBooking.id} (${selectedBooking.status}) - Nút hủy: ${canCancel ? 'Hiển thị' : 'Ẩn'}`);
    }
