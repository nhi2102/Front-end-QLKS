...
async function calculateTotal(maDatPhong) {
    try {
        // --- 1️⃣ Gọi API lấy tiền còn thiếu ---
        const remainingRes = await CheckoutAPI.getRemainingAmount(maDatPhong);
        // Giả sử API trả dạng { soTienThieu: 200000 }
        const remainingAmount =
            remainingRes.soTienThieu ? ?
            remainingRes.SoTienThieu ? ?
            remainingRes.sotienthieu ? ?
            0;
        ...
        const totalUnpaid =
            remainingAmount + serviceCharge + extraCharge + equipmentCompensation - discount;
