...
// ===============================
// Mở form chỉnh sửa từ modal xem chi tiết
// ===============================
function openEditCurrentCustomer() {
    if (!currentCustomer) {
        alert("Chưa chọn khách hàng nào để chỉnh sửa!");
        return;
    }
...
function updateStatistics() {
    ...
    const active = allCustomers.filter(c => c.activeBookings && c.activeBookings.length > 0).length;
    const vip = allCustomers.filter(c => c.bookings && c.bookings.length >= 5).length;
...
function applyFilters() {
    ...
    filteredCustomers = allCustomers.filter(c => {
        ...
        if (status === 'inactive' && c.activeBookings && c.activeBookings.length > 0) return false;
...
window.editCustomer = function(customerId) {
    ...
    const hotenEl = document.getElementById('editHoten');
    if (hotenEl) hotenEl.value = customer.hoten || '';
    const sdtEl = document.getElementById('editSdt');
    if (sdtEl) sdtEl.value = customer.sdt || '';
    const emailEl = document.getElementById('editEmail');
    if (emailEl) emailEl.value = customer.email || '';
    const diachiEl = document.getElementById('editDiachi');
    if (diachiEl) diachiEl.value = customer.diachi || '';
    const modal = document.getElementById('editCustomerModal');
    if (modal) modal.classList.add('show');
};
