...
function displayGridView(rooms) {
    ...
    container.innerHTML = rooms.map(r => `
        <div class="room-card">
            ...
            <div class="room-type">
                <i class="fas fa-bed"></i> ${r.tenLoaiPhong}
            </div>
            <div class="room-details">
                <i class="fas fa-money-bill"></i> ${formatCurrency(r.giaPhong)}/đêm
            </div>
            ${r.tenKhach ? `
                <div class="room-guest">
                    <strong>Khách:</strong> ${r.tenKhach}
                </div>` : ''}
        </div>
    `).join('');
}
...
function displayListView(rooms) {
    ...
    tbody.innerHTML = rooms.map(r => `
        <tr>
            ...
            <td>
                <span class="room-status ${getStatusClass(r.trangthai)}">
                    ${r.trangthai}
                </span>
            </td>
            <td>${r.tenKhach || '-'}</td>
...
function openRoomDetail(id) {
    ...
    document.getElementById('modalStatus').innerHTML = `
        <span class="room-status ${getStatusClass(selectedRoom.trangthai)}">
            ${selectedRoom.trangthai}
        </span>`;
    //  Thiết lập form cập nhật trạng thái
    const statusSelect = document.getElementById('newStatus');
    if (statusSelect) statusSelect.value = selectedRoom.trangthai || 'Trống';

    const noteInput = document.getElementById('statusNote');
    if (noteInput) noteInput.value = selectedRoom.ghichu || '';

    // Nếu có khách (đang sử dụng)
