...
function displaySearchResults(bookings) {
    ...
    bookings.forEach(b => {
                ...
                const name =
                    b.tenKhachHang ||
                    (b.khachhang && (b.khachhang.hoten || b.khachhang.tenkhachhang)) ||
                    (b.makhNavigation && (b.makhNavigation.hoten || b.makhNavigation.tenkhachhang)) ||
                    'Không rõ tên';

                // === THÔNG TIN PHÒNG / LOẠI PHÒNG ===
                const phong = b.phong || {};
                const loaiPhong = phong.loaiPhong || b.loaiPhong || {};
                const tenLoaiPhong = loaiPhong.tenloaiphong || loaiPhong.tenLoaiPhong || 'Không rõ loại';
                const soPhong = phong.sophong || phong.soPhong || 'N/A';

                // === GIÁ ===
                const giaCoBan = loaiPhong.giacoban || loaiPhong.giaCoBan || b.giaCoBan || 0;
                const giaKhuyenMai = loaiPhong.giaKhuyenMai || loaiPhong.giakhuyenmai || b.giaKhuyenMai || null;
                const tenVoucher = loaiPhong.tenVoucher || loaiPhong.tenvoucher || b.tenVoucher || null;

                // === TRẠNG THÁI ĐẶT PHÒNG ===
                const trangThai = b.trangthai || 'Không rõ';

                // === NGÀY NHẬN / TRẢ ===
                const ngayNhan = b.ngaynhanphong || b.checkin || b.ngayNhanPhong;
                const ngayTra = b.ngaytraphong || b.checkout || b.ngayTraPhong;

                // === HIỂN THỊ GIÁ (có khuyến mãi thì hiển thị 2 dòng) ===
                let priceHtml = `
            <div class="booking-price">
                <span class="price">${formatCurrency(giaCoBan)}</span>
                <span class="unit">/ đêm</span>
            </div>
        `;
                if (giaKhuyenMai && giaKhuyenMai < giaCoBan) {
                    priceHtml = `
                <div class="booking-price discount">
                    <span class="price-old">${formatCurrency(giaCoBan)}</span>
                    <span class="price-new">${formatCurrency(giaKhuyenMai)}</span>
                    <span class="unit">/ đêm</span>
                </div>
                ${tenVoucher ? `<div class="voucher-label"><i class="fas fa-tag"></i> ${tenVoucher}</div>` : ''}
            `;
        }
        ...
        card.innerHTML = `
            <div class="booking-card-header">
                <span>Mã ĐP: ${b.madatphong || b.maDatPhong}</span>
                <span class="booking-status ${trangThai.toLowerCase().replace(/\s/g, '-')}">${trangThai}</span>
            </div>
            <div class="booking-card-body">
                <div class="booking-info">
                    <div><i class="fas fa-user"></i> ${name}</div>
                    <div><i class="fas fa-bed"></i> Phòng ${soPhong} - ${tenLoaiPhong}</div>
                    <div><i class="fas fa-calendar"></i> ${formatDate(ngayNhan)} - ${formatDate(ngayTra)}</div>
                </div>
                ${priceHtml}
            </div>
        `;

        // === SỰ KIỆN CHỌN ĐẶT PHÒNG ===
