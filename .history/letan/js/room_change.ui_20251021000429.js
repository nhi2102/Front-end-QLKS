...
async function selectBooking(booking) {
    ...
    try {
        ...
        if (booking.trangthai === "Đang ở") {
            document.querySelector('input[value="changeDate"]').disabled = true;
            console.log(" Khách đang ở — chỉ được phép đổi phòng, không được đổi ngày!");
        } else {
            document.querySelector('input[value="changeDate"]').disabled = false;
        }
    } finally {
        showLoading(false);
    }
