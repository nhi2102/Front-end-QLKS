...
async function openCheckOutModal(booking) {
    ...
    // Lấy đúng tổng tiền chưa thanh toán (nếu không có thì = 0)
    let totalService = 0;
    if (services) {
        if (services.tongChuaThanhToan != null) {
            totalService = Number(services.tongChuaThanhToan);
        } else if (Array.isArray(services.chuaThanhToan)) {
            totalService = services.chuaThanhToan.reduce((s, v) => s + (v.thanhTien || 0), 0);
        }
    }
