...
async function showServiceHistory(bookingId) {
    ...
    // Populate header info for history modal (booking code, customer, rooms)
    try {
        let guest = (window.currentGuestsData || []).find(g => {
            const id = g.maDatPhong || g.madatphong || g.Madatphong || g.madat || g.maDatPhong;
            return String(id) === String(bookingId);
    ...
    body.innerHTML = data.map((item, i) => {
        ...
        // try to format a service date if present
        const usedDate = api.formatDate(item.ngaySuDung || item.ngaysudung || item.ngaydung || item.date || item.createdAt);
        return `
        <tr>
            <td>${i + 1}</td>
