...
function setupChangeTypeToggle() {
    ...
    // Thiết lập trạng thái ban đầu: chỉ hiện đổi phòng
    if (newDateSection) newDateSection.style.display = 'none';
...
async function selectBooking(booking) {
    ...
    try {
        ...
        // ✅ HIỆN phần giao diện trước
        const roomChangeSection = document.getElementById('roomChangeFormSection');
        if (roomChangeSection) roomChangeSection.style.display = 'block';

        // ✅ Gọi load phòng trống SAU khi giao diện đã hiện
        await loadAvailableRooms(booking);
    } catch (err) {
        console.error("Lỗi selectBooking:", err);
    } finally {
        showLoading(false);
    }
}
...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    //  Thêm xử lý tìm phòng trống theo ngày mới
    const btnFindNew = document.getElementById('btnFindByNewDate');
    if (btnFindNew) {
        btnFindNew.addEventListener('click', findRoomsByNewDate);
    }
...
async function findRoomsByNewDate() {
    ...
    showLoading(true);
    try {
        // Gọi API lấy danh sách phòng trống theo khoảng ngày
        const rooms = await RoomChangeAPI.fetchAvailableRoomsAPI(checkIn, checkOut);
        ...
        availableRooms = rooms;
        displayAvailableRooms(rooms); // ← Hiển thị phòng ra danh sách
        alert(`Đã tải ${rooms.length} phòng trống trong khoảng ${checkIn} → ${checkOut}`);
    } catch (err) {
...
function calculatePriceAdjustment() {
    ...
    // Lấy giá cũ từ phòng hiện tại
    const oldP = currentBooking.currentRoomPrice || 0;

    // Lấy giá mới từ phòng được chọn
    const newP =
