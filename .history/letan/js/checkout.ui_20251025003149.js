...
async function handleCheckout() {
    ...
    try {
        ...
        // 3️⃣ Sau khi checkout thành công → In hóa đơn
        try {
            // Gọi API lấy dữ liệu hóa đơn
            const invoice = await CheckoutAPI.getInvoiceByBookingId(bookingId);

            if (!invoice) {
                alert("Không tìm thấy dữ liệu hóa đơn để in!");
                return;
            }

            // Tạo nội dung HTML hóa đơn (có thể tùy chỉnh thêm logo, địa chỉ, v.v.)
            const invoiceHtml = `
        <html>
        <head>
        ...
        <body>
            ...
            <p><strong>Mã đặt phòng:</strong> #${invoice.maDatPhong || bookingId}</p>
            <p><strong>Khách hàng:</strong> ${invoice.tenKhachHang || currentBooking.tenKhachHang}</p>
            ...
            <table>
                ...
                <tbody>
                    ${(invoice.items || []).map(i => `
                        <tr>
                            <td>${i.tenMuc || i.tendichvu || i.tenphong || 'Mục khác'}</td>
                            <td>${i.soLuong || 1}</td>
                            <td>${formatCurrency(i.donGia || i.dongia || 0)}</td>
                            <td>${formatCurrency(i.thanhTien || i.thanhtien || 0)}</td>
                        </tr>
                    `).join('')}
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <td class="total">${formatCurrency(invoice.tongTien || invoice.tongtien || 0)}</td>
                    </tr>
                </tfoot>
            </table>
            <p style="text-align:center;margin-top:20px;">Cảm ơn quý khách, hẹn gặp lại!</p>
    ...
    console.error("Lỗi khi in hóa đơn:", printErr);
    alert("Không thể in hóa đơn, vui lòng kiểm tra lại!");
}
