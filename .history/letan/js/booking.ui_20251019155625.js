    ...
    try {
        ...
        if (roomType && roomType.trim() !== '') {
            rooms = rooms.filter(r =>
                (r.loaiPhong ? .maloaiphong === roomType) ||
                (r.loaiPhong ? .tenloaiphong ? .toLowerCase().includes(roomType.toLowerCase()))
            );
        }

        // --- Lọc theo số người ---
        if (guestCount > 0) {
            rooms = rooms.filter(r => (r.loaiPhong ? .songuoitoida || r.succhua || 0) >= guestCount);
        }
