...
function selectRoom(roomId) {
    ...
    const alreadySelected = selectedRooms.find(r => (r.maphong || r.maPhong) == roomId);

    // Nếu phòng đang được chọn => bỏ chọn
    if (alreadySelected) {
        selectedRooms = [];
        console.log("🚫 Bỏ chọn phòng:", roomId);
        document.getElementById('bookingFormSection').style.display = 'none';
    } else {
        // Reset toàn bộ và chọn phòng mới
        selectedRooms = [room];
        console.log("✅ Chọn phòng:", roomId);
