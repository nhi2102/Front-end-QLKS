...
function searchGuest() {
    try {
        ...
        // currentGuestsData is managed by service.api.js; fall back to reading displayed rows if missing
        const data = (window.currentGuestsData && Array.isArray(window.currentGuestsData) ? window.currentGuestsData : []);

        const filtered = data.filter(g => {
            const matchesCode = code ? (String(g.maDatPhong || g.madatphong || '')).toLowerCase().includes(code) : true;
            const matchesRoom = room ? (String((g.danhSachPhong || g.danhsachphong || []).map(p => p.soPhong || p.sophong).join(', '))).toLowerCase().includes(room) : true;
...
function resetSearch() {
    ...
    const codeEl = document.getElementById('searchBookingCode'); if (codeEl) codeEl.value = '';
    const roomEl = document.getElementById('searchRoomNumber'); if (roomEl) roomEl.value = '';
    const nameEl = document.getElementById('searchCustomerName'); if (nameEl) nameEl.value = '';
