...
async function submitBooking(e) {
    ...
    const gender = document.getElementById('customerGender').value.trim();
    ...
    try {
        // 1️⃣ Tìm hoặc tạo mới khách hàng
        ...
        // 2️⃣ Chuẩn bị dữ liệu chi tiết đặt phòng
        const details = selectedRooms.map(r => ({
            Maphong: r.maphong,
            Tongcong: r.loaiPhong?.giacoban || 0
        }));

        // 3️⃣ Xây dựng payload gửi API
        const payload = {
            TenKhachHang: name,
            Makh: customer.makh,
