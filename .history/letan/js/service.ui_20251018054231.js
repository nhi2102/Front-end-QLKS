...
function findDateInObject(obj, depth = 0) {
    ...
    const candidates = ['ngaySuDung','ngaysudung','ngaydung','ngaySD','NgaySuDung','createdAt','created_at','date','thoigian','time','ngay','ngaytao','$date','$numberLong','value','seconds','_seconds'];
...
async function showServiceHistory(bookingId) {
    ...
    if (!data || data.length === 0) {
        body.innerHTML = '<tr><td colspan="6">Chưa có dịch vụ nào</td></tr>';
        const noHistory = document.getElementById('noHistoryData');
        if (noHistory) noHistory.style.display = 'block';
        const totalCountEl = document.getElementById('totalServiceCount');
        const totalAmountEl = document.getElementById('totalHistoryAmount');
        if (totalCountEl) totalCountEl.textContent = '0';
        if (totalAmountEl) totalAmountEl.textContent = api.formatCurrency(0);
        return;
    }
    ...
    body.innerHTML = data.map((item, i) => {
        const qty = item.soLuong || item.soluong || item.quantity || 1;
        const unit = item.donGia || item.dongia || item.unitPrice || item.donGiaVnd || 0;
        const lineTotal = item.thanhTien || item.thanhTien || (unit * qty);
        totalCount += Number(qty) || 0;
        totalAmount += Number(lineTotal) || 0;
    // định dạng ngày sử dụng - try to discover a date-like field in the item
    const candidateDate = findDateInObject(item) || item.ngaySuDung || item.ngaysudung || item.ngaydung || item.date || item.createdAt;
    let usedDate = api.formatDate(candidateDate);
    // fallback: try letting api.formatDate scan the whole item (it understands wrappers like $date, seconds, etc.)
    if (usedDate === 'N/A') {
        const alt = api.formatDate(item);
        if (alt !== 'N/A') {
            usedDate = alt;
            console.debug('showServiceHistory: used fallback formatDate(item) for record', item);
        }
    }
    if (usedDate === 'N/A') {
        // Instrumentation to help debugging: log the raw record and candidate date
        console.debug('showServiceHistory: still N/A after fallbacks', { item, candidateDate });
    }
        return `
        <tr>
            <td>${i + 1}</td>
            <td>${item.tenDichVu || item.tendv || item.serviceName || 'N/A'}</td>
            <td>${qty}</td>
            <td>${api.formatCurrency(unit)}</td>
            <td>${api.formatCurrency(lineTotal)}</td>
            <td>${usedDate}</td>
        </tr>`;
    }).join('');
    ...
    if (totalAmountEl) totalAmountEl.textContent = api.formatCurrency(totalAmount);
}
