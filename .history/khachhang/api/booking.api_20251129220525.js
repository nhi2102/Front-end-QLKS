...
async function bookRooms(bookingData) {
    ...
    const body = {
        ...
        LuuChiTietDiem: true // Yêu cầu backend lưu điểm vào chitiethoadon
    };
...
async function saveServices(maDatPhong, rooms) {
    ...
    for (const room of rooms) {
        ...
        if (!room.services || !room.services.length) {
            console.log(" Phòng không có dịch vụ, bỏ qua");
            continue;
        }
        ...
        for (const service of room.services) {
            try {
                const payload = {
                    ...
                    Manv: null // Khách hàng đặt online, không có nhân viên
                };
