...
async function bookRooms(bookingData) {
    ...
    for (const r of rooms) {
        ...
        giaPhongs.push(r.price); // giá/đêm 1 phòng
    ...
    // ✅ 1. Lưu dịch vụ nếu có
    await saveServices(result.maDatPhong, rooms);

    // ✅ 2. Chờ để backend commit + trigger cập nhật xong
    await new Promise(r => setTimeout(r, 800)); // <-- tăng lên 0.8s để chắc chắn

    // ✅ 3. Đọc lại hóa đơn (tối đa 3 lần để đảm bảo lấy đúng tổng mới)
    let invoice = null;
    for (let i = 0; i < 3; i++) {
        try {
            invoice = await getInvoiceInfo(result.hoaDonId);
            if (invoice && (invoice.tongtien != null || invoice.Tongtien != null)) break;
        } catch (e) {
            console.warn(`Lần ${i + 1} đọc hóa đơn lỗi:`, e);
        }
        await new Promise(r => setTimeout(r, 5000));
    }

    // ✅ 4. Lấy tổng tiền đúng nhất (ưu tiên theo thứ tự)
    let tongHoaDon = 0;
    if (invoice && invoice.tongtien !== undefined && invoice.tongtien !== null) {
        tongHoaDon = invoice.tongtien;
    } else if (invoice && invoice.Tongtien !== undefined && invoice.Tongtien !== null) {
        tongHoaDon = invoice.Tongtien;
    } else if (result && result.tongTien !== undefined && result.tongTien !== null) {
        tongHoaDon = result.tongTien;
    } else {
        tongHoaDon = 0;
    }

    console.log("💰 Tổng tiền hóa đơn cuối cùng (sau trigger):", tongHoaDon);
...
const KhachHangAPI = {
    async getById(id) {
        const res = await fetch(`https://localhost:7076/api/Khachhangs/${id}`);
        if (!res.ok) throw new Error('Không thể lấy thông tin khách hàng');
        return res.json();
    },

    async updateDiem(id, newDiem) {
        const res = await fetch(`/api/khachhang/update-diem`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id, diem: newDiem })
        });
