    ...
    if (profileForm) {
        ...
        profileForm.addEventListener('submit', async(e) => {
            ...
            try {
                ...
                console.log("Dữ liệu gửi đi:", updateDto);
                console.log("URL API:", `${API_BASE}/Khachhangs/${customerId}`);
                ...
                // Kiểm tra content-type để xử lý response đúng cách
                ...
                if (contentType && contentType.includes('application/json')) {
                    ...
                    if (!response.ok) {
                        errorMessage = result.message || result.error || 'Không thể cập nhật.';
                    }
                } else {
                    // Response là plain text hoặc không phải JSON
                    const textResult = await response.text();
                    if (!response.ok) {
                        errorMessage = textResult || 'Không thể cập nhật.';
                    }
                }

                if (response.ok) {
                    const successMessage = result?.message || 'Cập nhật thông tin khách hàng thành công!';
