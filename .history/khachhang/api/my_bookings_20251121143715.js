...
async function submitReview(reviewData) {
    try {
        ...
        if (!reviewData.danhgia || reviewData.danhgia.trim() === '') {
            throw new Error('Vui lòng nhập đánh giá');
        }

        // Chuẩn bị dữ liệu gửi API theo cấu trúc mới
        const reviewApiData = {
            Makh: parseInt(makhachhang),
            Madatphong: parseInt(reviewData.madatphong),
            Sosao: parseInt(reviewData.sosao),
            Danhgia: reviewData.danhgia.trim(),
            MakhNavigation: {},  // Thêm object rỗng để tránh validation error
            MadatphongNavigation: {}  // Thêm object rỗng để tránh validation error
        };

        console.log('📤 Gửi đánh giá tới API:', reviewApiData);

        // Gửi request tới API Reviews/submit
        ...
        if (!response.ok) {
            ...
            console.error('❌ Chi tiết lỗi:', JSON.stringify(result, null, 2));
            console.error('❌ Payload đã gửi:', JSON.stringify(reviewApiData, null, 2));
            throw new Error(result.error || result.message || 'Không thể gửi đánh giá');
        }
        ...
        return {
            success: true,
            message: result.message || 'Cảm ơn bạn đã đánh giá!',
            data: result
        };
