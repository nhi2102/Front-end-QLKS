...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        // Kiểm tra trùng CCCD
        const cccdExists = await checkDuplicateData(formData.cccd, null, null);
        if (cccdExists) {
            showError('Số CCCD/CMND đã được đăng ký!');
            return;
        }

        // Kiểm tra trùng SĐT
        const sdtExists = await checkDuplicateData(null, formData.sodienthoai, null);
        if (sdtExists) {
        ...
        const emailExists = await checkDuplicateData(null, null, formData.email);
        if (emailExists) {
    ...
    async function checkDuplicateData(cccd, sdt, email) {
        try {
            ...
            const response = await fetch(`${API_BASE}/Khachhangs/timkiem?${params.toString()}`);
            if (response.ok) {
