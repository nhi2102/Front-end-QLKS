...
async function loadServices() {
    ...
    try {
        ...
        // Kiểm tra nếu là lỗi timeout/network
        if (error.name === 'AbortError' || error.message.includes('Failed to fetch')) {
            console.warn('API dịch vụ timeout, sử dụng dữ liệu fallback');

            // Sử dụng dữ liệu dịch vụ mẫu
            const fallbackServices = getFallbackServices();
            renderFallbackServices(fallbackServices, servicesList);

            // Hiển thị warning nhỏ
            const warning = document.createElement('div');
            warning.style.cssText = 'text-align: center; padding: 10px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px; margin-bottom: 15px; color: #856404; font-size: 13px;';
            warning.innerHTML = ' Kết nối chậm, hiển thị dịch vụ mẫu';
            servicesList.insertBefore(warning, servicesList.firstChild);
        } else {
