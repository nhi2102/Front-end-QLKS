...
// Hàm kiểm tra trùng lặp khi cập nhật (bỏ qua chính khách hàng hiện tại)
async function checkDuplicateInUpdate(currentId, sdt, cccd, email) {
    try {
        // Kiểm tra SĐT
        if (sdt) {
            const sdtResponse = await fetch(`${API_BASE}/Khachhangs/timkiem?sdt=${sdt}`);
            if (sdtResponse.ok) {
                const sdtResult = await sdtResponse.json();
                if (Array.isArray(sdtResult) && sdtResult.length > 0) {
                    const duplicate = sdtResult.find(k => k.makh != currentId);
        ...
        // Kiểm tra CCCD
        if (cccd) {
            const cccdResponse = await fetch(`${API_BASE}/Khachhangs/timkiem?cccd=${cccd}`);
            if (cccdResponse.ok) {
                const cccdResult = await cccdResponse.json();
                if (Array.isArray(cccdResult) && cccdResult.length > 0) {
                    const duplicate = cccdResult.find(k => k.makh != currentId);
        ...
        // Kiểm tra Email
        if (email) {
            const emailResponse = await fetch(`${API_BASE}/Khachhangs/timkiem?email=${email}`);
            if (emailResponse.ok) {
                const emailResult = await emailResponse.json();
                if (Array.isArray(emailResult) && emailResult.length > 0) {
                    const duplicate = emailResult.find(k => k.makh != currentId);
    ...
    if (profileForm) {
        ...
        profileForm.addEventListener('submit', async(e) => {
            ...
            try {
                ...
                const duplicateCheck = await checkDuplicateInUpdate(customerId, sdt, cccd, email);
                if (duplicateCheck) {
                ...
                const updateData = {
                    ...
                    Diemthanhvien: user.diemthanhvien || 0,
