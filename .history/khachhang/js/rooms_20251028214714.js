...
document.addEventListener("DOMContentLoaded", function() {
    ...
    try {
        ...
        if (roomTypeInfo) {
            ...
            document.getElementById('modalRoomSize').textContent = roomTypeInfo.dientich ? `${roomTypeInfo.dientich} m²` : 'Diện tích không xác định';
            ...
            console.log('hinhAnhPhongs:', roomTypeInfo.hinhAnhPhongs);
            console.log('hinhAnh:', roomTypeInfo.hinhAnh);

            // Kiểm tra cấu trúc dữ liệu API trả về cho hình ảnh
            if (roomTypeInfo.hinhAnhPhongs && roomTypeInfo.hinhAnhPhongs.length > 0) {
                imageUrls = roomTypeInfo.hinhAnhPhongs.map(img => img.duongDan || img.url || ''); 
            } else if (roomTypeInfo.hinhAnh) {
                // Nếu không có hinhAnhPhongs, sử dụng ảnh chính
