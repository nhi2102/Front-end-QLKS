...
async function displayLoyaltyPointsImmediate(customerData) {
    ...
    if (pointsInput) {
        ...
        pointsInput.placeholder = `Tối đa ${currentPoints.toLocaleString('vi-VN')} điểm`;
    }
}/**
...
function handleApplyPoints() {
    ...
    if (pointsToUse > maxPoints) {
        alert(`Bạn chỉ có ${maxPoints.toLocaleString('vi-VN')} điểm!`);
        pointsInput.value = maxPoints;
        return;
    }
    
    // Tính tiền giảm (1000 điểm = 1,000 VNĐ, tức 1 điểm = 1 VNĐ)
    const discountAmount = pointsToUse;
    
    // Hiển thị giảm giá
    showPointsDiscount(discountAmount, pointsToUse);
    ...
    console.log(`Áp dụng ${pointsToUse} điểm, giảm ${discountAmount.toLocaleString('vi-VN')} VNĐ`);
}
...
function updateTotalPrice() {
    ...
    // Lấy số tiền giảm từ điểm
    const discountSection = document.getElementById('points-discount');
    const discountValueEl = document.getElementById('points-discount-value');
    let discountAmount = 0;
    
    if (discountSection && discountSection.style.display !== 'none' && discountValueEl) {
        const discountText = discountValueEl.textContent.replace(/[^\d]/g, '');
    ...
    // Cập nhật hiển thị
    const totalElements = document.querySelectorAll('.final-price');
    totalElements.forEach(el => {
        el.textContent = `${finalTotal.toLocaleString('vi-VN')} VNĐ`;
    });
    
    console.log(`Tổng tiền cập nhật: ${finalTotal.toLocaleString('vi-VN')} VNĐ (gốc: ${originalTotal.toLocaleString('vi-VN')}, giảm: ${discountAmount.toLocaleString('vi-VN')})`);
}
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        const vnpayUrl = await BookingAPI.createVNPayURL({
            ...
            amount: result.tongTien,  // đã là giá sau giảm
...
document.getElementById('continuePayment').addEventListener('click', () => {
    ...
    resolve(); // cho phép tiếp tục sang VNPay
});