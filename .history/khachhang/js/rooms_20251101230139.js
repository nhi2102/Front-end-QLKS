...
document.addEventListener("DOMContentLoaded", function() {
    ...
    roomsList.innerHTML = ""; // Xóa toàn bộ nội dung tĩnh
});
...
function loadSearchData(updateDatePicker = false) {
    ...
    if (searchData) {
        ...
        const nights = parseInt(searchData.nights, 10) || 1; // nights là số đêm
        const days = nights + 1; // nếu bạn muốn hiển thị số ngày (tính cả ngày check-in + checkout)
        const durationText = `${days} ngày ${nights} đêm`;
...
document.addEventListener("DOMContentLoaded", function() {
            ...
            function initializeCartFunctionality() {
                ...
                // Cập nhật text button theo số phòng còn lại từ giỏ hàng hiện tại
                setTimeout(() => {
                    updateAllRoomButtonsText();
                }, 500); // Đợi DOM render xong
            }
            ...
            //     // Kiểm tra số lượng phòng cùng loại đã chọn trong giỏ
            //     const roomsOfSameType = cart.filter(item => item.roomType === roomType);
            //     const availableRooms = parseInt(button.dataset.availableRooms) || 0;

            //     // Nếu đã chọn đủ số phòng trống, không cho chọn thêm
            //     if (roomsOfSameType.length >= availableRooms) {
            //         button.textContent = 'Hết phòng!';
            //         button.style.backgroundColor = '#dc3545';
            //         setTimeout(() => {
            //             button.textContent = `Chọn Phòng (${availableRooms - roomsOfSameType.length} trống)`;
            ...
            //         // TẮT ĐÁNH DẤU - Không đánh dấu phòng đã chọn để cho phép chọn nhiều lần
            //         // roomItem.classList.add('in-cart');

            //         // Reset button - chỉ hiện hiệu ứng thêm thành công rồi về lại bình thường
            ...
            //         // Reset button sau 1 giây và cập nhật số phòng còn lại
            //         setTimeout(() => {
            //             button.classList.remove('added');
            //             updateRoomButtonText(button, roomType);
            //         }, 1000);
            ...
            function handleAddToCart(button) {
                ...
                // === SỬA ĐỔI QUAN TRỌNG ===
                // Lấy giá TỪ DATASET của nút bấm, không phải từ text
                // 'finalPrice' này đã được tính toán ở hàm renderRoomTypes
                // (đã là giá KM nếu có, hoặc giá gốc nếu không có)
                ...
                // === SỬA ĐỔI QUAN TRỌNG ===
                // Lấy MALOAIPHONG (room-type-id) thay vì roomId (vì bạn đang chọn theo loại)
                ...
                setTimeout(() => {
                    ...
                    cart.push({
                        roomType: roomType,
                        roomTypeId: roomTypeId, // === SỬA: Lưu MALOAIPHONG ===
                        title: roomTitle,
                        price: price, // Giá này là giá cuối cùng (đã KM nếu có)
                        quantity: 1,
                        services: []
                    });
            ...
            // Hàm cập nhật text button theo số phòng còn lại
            function updateRoomButtonText(button, roomType) {
                const availableRooms = parseInt(button.dataset.availableRooms) || 0;
                const roomsOfSameType = cart.filter(item => item.roomType === roomType);
                const remainingRooms = availableRooms - roomsOfSameType.length;
                ...
                if (remainingRooms <= 0) {
                    ...
                    button.textContent = `Chọn Phòng (${remainingRooms} trống)`;
                    button.disabled = false;
                    button.style.backgroundColor = '';
                }
            }

            // Hàm cập nhật tất cả button text khi có thay đổi giỏ hàng
            function updateAllRoomButtonsText() {
            ...
            window.updateAllRoomButtonsText = updateAllRoomButtonsText;
            ...
            function updateCartDisplay() {
                ...
                // === SỬA ĐỔI QUAN TRỌNG: Lấy số đêm ===
                // Lấy số đêm từ biến global, nếu không có thì mặc định là 1
                const nights = window.searchNights || 1;
                // ===================================
                ...
                if (cart.length === 0) {
                    ...
                    // === SỬA ĐỔI QUAN TRỌNG: Tính tổng tiền phòng (NHÂN VỚI SỐ ĐÊM) ===
                    const totalRooms = cart.reduce((sum, item) => sum + item.quantity, 0);
                    const roomTotal = cart.reduce((sum, item) => {
                        // Lấy giá phòng (đã KM) * số lượng (thường là 1) * SỐ ĐÊM
                        return sum + (item.price * item.quantity * nights);
                    }, 0);
                    // ==========================================================

                    // Tính tổng tiền dịch vụ (Dịch vụ KHÔNG nhân số đêm)
                    const servicesTotal = cart.reduce((sum, item) => {
                        if (item.services && item.services.length > 0) {
                            const roomServicesTotal = item.services.reduce((serviceSum, service) => {
                                return serviceSum + (service.dongia * service.soluong);
                    ...
                    if (totalAmountElement) {
                        ...
                        // Hiển thị chi tiết tiền phòng (đã nhân số đêm)
                        let roomTotalHtml = `
                ${formatPrice(roomTotal)} VNĐ 
                <small style="display: block; font-size: 11px; font-weight: 400; color: #555;">
                    (Tiền phòng cho ${nights} đêm)
                </small>
            `;
                ...
                `;
                            ...
                            // Chỉ có tiền phòng
                            totalAmountElement.innerHTML = roomTotalHtml;
                        }
                    }
                    if (bookNowBtn) bookNowBtn.disabled = false;

                    // Render cart items (gọi hàm renderCartItems đã có)
                    renderCartItems();
                }
            }
    ...
    // === SỬA ĐỔI QUAN TRỌNG: Hiển thị giá đã nhân số đêm ===
    cartItemDiv.innerHTML = `
        <div class="cart-item-info">
            <div class="cart-item-title">${item.title}</div>
            <div class="cart-item-price">
                ${formatPrice(item.price * nights)} VNĐ 
                <small style="color:#555; font-weight: 400;">(${nights} đêm)</small>
            </div>
            ${servicesHTML}
            <button class="add-service-btn" onclick="openServicesForRoom(${index})" 
                    style="margin-top: 8px; padding: 5px 12px; font-size: 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 4px; cursor: pointer;">
                <i class="fas fa-plus"></i> ${item.services && item.services.length > 0 ? 'Sửa dịch vụ' : 'Thêm dịch vụ'}
            </button>
        </div>
        <div class="cart-item-actions">
            <button class="remove-item-btn" onclick="removeFromCart(${index})" title="Xóa khỏi giỏ">
    ...
    window.updateQuantity = function(index, change) {
        if (cart[index]) {
            ...
            if (cart[index].quantity <= 0) {
                ...
                updateCartDisplay();
                updateAllRoomButtonsText(); // Cập nhật text button sau khi thay đổi số lượng
                saveCart(); //
            }
        }
    };
    ...
    window.removeFromCart = function(index) {
        if (cart[index]) {
            ...
            updateCartDisplay();
            updateAllRoomButtonsText(); // Cập nhật text button sau khi xóa khỏi giỏ
            saveCart(); //
        }
    };
    ...
    //     console.log('Bắt đầu quá trình đặt phòng...', cart);
    ...
    //     // 5. Disable nút đặt phòng và hiển thị loading
    //     const bookNowBtn = document.getElementById('book-now-btn');
    //     if (bookNowBtn) {
    //         bookNowBtn.disabled = true;
    //         bookNowBtn.textContent = 'Đang xử lý...';
    //     }

    //     // 6. Xóa giỏ hàng sau khi đặt phòng thành công
    //     cart = [];
    //     localStorage.removeItem('cartItems');
        
    //     // Cập nhật hiển thị giỏ hàng ngay lập tức
    //     updateCartDisplay();
        
    //     console.log('Đã xóa giỏ hàng sau khi đặt phòng thành công');

    //     // 7. Chuyển hướng sang trang đặt phòng để nhập thông tin khách hàng (với delay nhỏ)
    //     setTimeout(() => {
...
function handleBooking() {
    ...
    // 2. Tính toán tổng giá và số phòng (NHÂN VỚI SỐ ĐÊM)
    const roomTotal = cart.reduce((sum, item) => sum + (item.price * item.quantity * nights), 0);
    const servicesTotal = cart.reduce((sum, item) => {
        if (item.services && item.services.length > 0) {
            return sum + item.services.reduce((serviceSum, service) => serviceSum + (service.dongia * service.soluong), 0);
        }
        return sum;
    }, 0);
    ...
    const searchData = JSON.parse(localStorage.getItem('hotelSearch')) || {};
    const bookingData = {
        rooms: cart, // cart này đã chứa 'roomTypeId' và 'price' (giá 1 đêm)
        searchInfo: searchData, // Chứa 'dateRange', 'nights', 'guestCount'
        totalRooms: totalRooms,
        totalRoomPrice: roomTotal, // Tổng tiền phòng (đã nhân số đêm)
        totalServicePrice: servicesTotal, // Tổng tiền dịch vụ
        totalPrice: totalPrice, // Tổng cuối cùng
        bookingTime: new Date().toISOString()
    };
    ...
    // 7. Chuyển hướng sang trang đặt phòng
    setTimeout(() => {
        window.location.href = 'cus_info_booking.html';
    }, 500);
}
    ...
    function initializeAmenitiesModal() {
        ...
        async function openAmenitiesModal(roomTypeId) { 
    const targetMaLoaiPhong = parseInt(roomTypeId.replace('roomtype-', ''), 10); 

    try {
        // Lấy thông tin loại phòng từ global variable (window.roomsData)
        const roomsData = window.roomsData || []; 
        const roomTypeInfo = roomsData.find(rt => rt.maloaiphong == targetMaLoaiPhong); 

        if (roomTypeInfo) {
            console.log('Room Type Info:', roomTypeInfo);
            
            // Cập nhật thông tin phòng
            document.getElementById('modalRoomTitle').textContent = roomTypeInfo.tenloaiphong || 'Tên phòng không xác định';
            document.getElementById('modalRoomSize').textContent = roomTypeInfo.songuoitoida ? `${roomTypeInfo.songuoitoida} người` : 'Sức chứa không xác định';
            document.getElementById('modalRoomDescription').textContent = roomTypeInfo.mota || 'Không có mô tả';

            // Set images (Lấy đường dẫn URL)
            let imageUrls = ['../assets/img/room-default.jpg']; // Ảnh mặc định

            console.log('hinhAnh array:', roomTypeInfo.hinhAnh);

            // Kiểm tra cấu trúc dữ liệu API trả về cho hình ảnh
            if (roomTypeInfo.hinhAnh && roomTypeInfo.hinhAnh.length > 0) {
                // roomTypeInfo.hinhAnh là array các object có thuộc tính duongdan hoặc duongDan
                imageUrls = roomTypeInfo.hinhAnh.map(img => img.duongdan || img.duongDan || img.url || ''); 
            } else {
                // Fallback sử dụng ảnh mặc định
                imageUrls = ['../assets/img/room-default.jpg'];
            }
            
            // Lọc bỏ các URL rỗng nếu có lỗi
            imageUrls = imageUrls.filter(url => url && url.trim() !== '');
            if(imageUrls.length === 0) imageUrls = ['../assets/img/room-default.jpg']; // Fallback nếu lọc hết
            ...
            updateModalImage();
             ...
             console.error(`Không tìm thấy loại phòng với mã: ${targetMaLoaiPhong}`);
             // Có thể hiển thị thông báo lỗi trong modal
             document.getElementById('modalRoomTitle').textContent = 'Lỗi';
             document.getElementById('modalRoomDescription').textContent = 'Không tìm thấy thông tin loại phòng.';
        }
    } catch (error) {
        console.error('Lỗi khi lấy thông tin loại phòng:', error);
    }

    // Hiển thị modal (code cũ giữ nguyên)
    const modal = document.getElementById('amenitiesModal');
...
// (Hàm updateModalImage, closeModal, showNextModalImage, showPrevModalImage giữ nguyên)

        // Cập nhật hình ảnh trong modal
        function updateModalImage() {
            console.log('updateModalImage called');
            console.log('currentModalImages:', currentModalImages);
            console.log('currentModalImageIndex:', currentModalImageIndex);
            console.log('modalMainImage element:', modalMainImage);
            ...
            if (currentModalImages.length > 0) {
                ...
                console.log('Image src after setting:', modalMainImage.src);
            } else {
                console.log('No images in currentModalImages array');
            }
        }
...
function updateSearchDataFromWidget() {
    ...
    // Tính toán số đêm
    // Tính toán số đêm chính xác
let nights = 1;
let formattedDates = dateRange;

if (dateRange.includes(' - ')) {
    const dates = dateRange.split(' - ').map(s => s.trim());
    if (dates.length === 2) {
        const [d1, d2] = dates;
        const [dd1, mm1, yy1] = d1.split('/').map(Number);
        const [dd2, mm2, yy2] = d2.split('/').map(Number);

        // Tạo Date bằng new Date(yyyy, mm-1, dd) để tránh timezone
        const startDate = new Date(yy1, mm1 - 1, dd1);
        const endDate   = new Date(yy2, mm2 - 1, dd2);

        // Nếu endDate <= startDate => coi như 1 đêm (bảo vệ)
        if (endDate <= startDate) {
            nights = 1;
        } else {
            const msPerDay = 1000 * 60 * 60 * 24;
            // Hiệu số ngày chính xác (ví dụ 20->23 = 3)
            nights = Math.round((endDate - startDate) / msPerDay);
            if (nights < 1) nights = 1;
        }

        formattedDates = `${d1} - ${d2}`;
    }
}
...
window.openServicesForRoom = function(roomIndex) {
    ...
    if (policyModal) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                console.log('Gọi loadServices cho phòng:', roomIndex);
                
                // Kiểm tra xem element services-list có tồn tại không
                const servicesList = document.getElementById('services-list');
                console.log('Services list element:', servicesList);
                
                // Gọi loadServices từ window (đã được export từ room.js)
                if (typeof window.loadServices === 'function') {
                    console.log('Gọi window.loadServices()');
                    window.loadServices().then(() => {
                        console.log('Services loaded, now restoring for room');
                    ...
                    console.error('window.loadServices function không tồn tại! Có thể room.js chưa được load.');
                    restoreServicesForRoom(roomIndex);
                }
            }, 200);
        }, 10);
...
// Cập nhật button text khi trang load hoàn tất
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        if (typeof updateAllRoomButtonsText === 'function') {
            updateAllRoomButtonsText();
        }
    }, 1000); // Đợi tất cả dữ liệu load xong
});