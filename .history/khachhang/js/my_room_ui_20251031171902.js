...
document.addEventListener('DOMContentLoaded', async function() {
        ...
        // ƯU TIÊN DÙNG TỔNG TIỀN ĐÃ TÍNH TỪ TONGCONG (đã giảm giá)
        if (tongTienPhongAll === 0 && tongTienDichVuAll === 0 && tongTienTatCaPhong > 0) {
            console.log(' ✅ Dùng tongTienTatCaPhong từ hóa đơn đã giảm:', formatCurrency(tongTienTatCaPhong));
            
            // Tính lại tỷ lệ phòng/dịch vụ để phân bổ đúng
            let tongTienPhongGoc = 0;
            let tongTienDichVuGoc = 0;
            ...
            chiTiet.forEach(ct => {
                ...
                tongTienDichVuGoc += tienDichVuPhong;
            });
            
            // Phân bổ tongTienTatCaPhong theo tỷ lệ gốc
            const tongGoc = tongTienPhongGoc + tongTienDichVuGoc;
            if (tongGoc > 0) {
                const tylePhong = tongTienPhongGoc / tongGoc;
                const tyleDichVu = tongTienDichVuGoc / tongGoc;
                
                tongTienPhongAll = Math.round(tongTienTatCaPhong * tylePhong);
                tongTienDichVuAll = Math.round(tongTienTatCaPhong * tyleDichVu);
            } else {
                tongTienPhongAll = tongTienTatCaPhong;
                tongTienDichVuAll = 0;
            }
            
            console.log(` 📊 Phân bổ: Phòng ${formatCurrency(tongTienPhongAll)} + Dịch vụ ${formatCurrency(tongTienDichVuAll)} = ${formatCurrency(tongTienPhongAll + tongTienDichVuAll)}`);
        }
