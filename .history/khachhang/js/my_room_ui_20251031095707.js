...
document.addEventListener('DOMContentLoaded', async function() {
            ...
            async function getHoaDonIdFromDatPhong(maDatPhong) {
                try {
                    console.log(`🔍 Tìm mã hóa đơn từ đặt phòng: ${maDatPhong}`);
                    const response = await fetch(`https://localhost:7076/api/Datphongs/${maDatPhong}`);
            ...
            async function createBookingCard(booking, isUpcoming) {
                ...
                let hoaDonId = booking.mahoadon || booking.MaHoaDon || booking.hoadonId || 
                               booking.HoaDonId || booking.hoaDonId || booking.maHoaDon;
                let tongTienHoaDon = 0;
                
                console.log(`🎯 Mã hóa đơn từ booking: ${hoaDonId}`);
                
                // Nếu không có mã hóa đơn, tìm từ mã đặt phòng
                if (!hoaDonId && madatphong) {
                    console.log(`🔍 Không có mã hóa đơn, tìm từ mã đặt phòng: ${madatphong}`);
                    hoaDonId = await getHoaDonIdFromDatPhong(madatphong);
                }
                
                if (hoaDonId) {
                    console.log(`✅ Sẽ gọi API với mã hóa đơn: ${hoaDonId}`);
                    tongTienHoaDon = await getTongTienFromHoaDon(hoaDonId);
                } else {
                    console.warn('⚠️ Không tìm thấy mã hóa đơn nào');
                }
                    ...
                    <div class="room-item-info">
                        ...
                        <div class="room-pricing">
                            <div class="price-detail">
                                ...
                                <span class="price-value">${formatCurrency(giaPhong)}/đêm (đã giảm) × ${nights} đêm = <strong>${formatCurrency(tienPhong)}</strong></span>
                            </div>
                            ${servicesHTML}
