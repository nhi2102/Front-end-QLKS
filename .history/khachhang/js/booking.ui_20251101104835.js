    ...
    try {
        ...
        if (result.hoaDonId) {
            try {
                ...
                // LẤY TỔNG TIỀN HÓA ĐƠN (không phải tổng tiền đặt phòng)
                finalAmount = Number(hoaDonData.tongtien || hoaDonData.TongTien || 0);
                finalAmount = Math.round(finalAmount); // đảm bảo số nguyên
                console.log('💰 Tổng tiền hóa đơn chính xác:', finalAmount);

                
                console.log('💰 Tổng tiền HÓA ĐƠN truyền VNPay:', formatPrice(finalAmount));
