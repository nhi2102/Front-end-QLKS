...
document.addEventListener("DOMContentLoaded", function() {
    ...
    try {
        ...
        if (roomTypeInfo) {
            // ... (cập nhật title, size, description) ...
...
if (roomTypeInfo) {
        ...
        // --- SỬA LẠI PHẦN LẤY ẢNH ---
        let imageUrls = [];
        let mainImageUrl = '../assets/img/room-default.jpg'; // Ảnh mặc định

        // 1. Ưu tiên lấy ảnh chính (giống như cách bạn lấy ảnh cho card phòng)
        if (roomTypeInfo.hinhAnh) { // Giả sử 'hinhAnh' là ảnh chính dạng chuỗi URL
            mainImageUrl = roomTypeInfo.hinhAnh;
            imageUrls.push(mainImageUrl); // Đặt ảnh chính vào đầu mảng
        } else if (roomTypeInfo.hinhAnhPhongs && roomTypeInfo.hinhAnhPhongs.length > 0) {
            // Nếu không có ảnh chính riêng, lấy ảnh đầu tiên trong mảng ảnh phụ
            // Đảm bảo lấy đúng thuộc tính URL (ví dụ: duongDan)
            mainImageUrl = roomTypeInfo.hinhAnhPhongs[0].duongDan || roomTypeInfo.hinhAnhPhongs[0].url || mainImageUrl;
             imageUrls.push(mainImageUrl); // Đặt ảnh này vào đầu mảng
        } else {
             imageUrls.push(mainImageUrl); // Chỉ có ảnh mặc định
        }


        // 2. Lấy các ảnh phụ còn lại (nếu có và khác ảnh chính đã lấy)
        if (roomTypeInfo.hinhAnhPhongs && roomTypeInfo.hinhAnhPhongs.length > 0) {
            roomTypeInfo.hinhAnhPhongs.forEach(img => {
                const imgUrl = img.duongDan || img.url || '';
                // Chỉ thêm vào nếu URL hợp lệ VÀ chưa có trong mảng (tránh trùng lặp ảnh chính)
        ...
        // Lọc bỏ URL rỗng lần cuối (phòng trường hợp lỗi)
        imageUrls = imageUrls.filter(url => url);
        if(imageUrls.length === 0) imageUrls = ['../assets/img/room-default.jpg']; // Fallback cuối


        console.log('Ảnh cho modal (Ảnh chính đầu tiên):', imageUrls); // DEBUG

        currentModalImages = imageUrls; // Mảng các chuỗi URL, ảnh chính ở vị trí 0
        currentModalImageIndex = 0; // Luôn bắt đầu từ ảnh chính
        updateModalImage(); // Gọi hàm cập nhật ảnh
        } else {
    ...
    if (policyModal) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                loadServices(); // Gọi lại loadServices
