    ...
    try {
        ...
        if (roomTypeInfo) {
            ...
            if (roomTypeInfo.hinhAnhPhongs && roomTypeInfo.hinhAnhPhongs.length > 0) {
                // Giả sử hinhAnhPhongs là mảng object [{ duongDan: 'url1' }, { duongDan: 'url2' }]
                // Dùng .map() để lấy ra mảng CHUỖI URL
                imageUrls = roomTypeInfo.hinhAnhPhongs.map(img => img.duongDan || img.url || ''); // Lấy thuộc tính chứa URL (vd: duongDan, url)
            } else if (roomTypeInfo.hinhAnh) {
                // Nếu chỉ có 1 ảnh chính dạng chuỗi
                imageUrls = [roomTypeInfo.hinhAnh];
            }
            // Lọc bỏ các URL rỗng nếu có lỗi
            imageUrls = imageUrls.filter(url => url);
            if(imageUrls.length === 0) imageUrls = ['../assets/img/room-default.jpg']; // Fallback nếu lọc hết

            currentModalImages = imageUrls; // ✅ Đảm bảo đây là mảng các chuỗi URL
...
function updateModalImage() {
    // Lấy thẻ img hiển thị chính (Đảm bảo ID này đúng với HTML của bạn)
    const modalMainImage = document.getElementById('modalMainImage');

    if (!modalMainImage) {
         console.error("LỖI: Không tìm thấy phần tử img '#modalMainImage'!");
         return;
    }

    if (currentModalImages && currentModalImages.length > 0 && currentModalImageIndex < currentModalImages.length) {
        // Lấy CHUỖI URL tại vị trí hiện tại
        const imageUrl = currentModalImages[currentModalImageIndex];

        // Gán chuỗi URL vào thuộc tính src
        modalMainImage.src = imageUrl;
        modalMainImage.alt = `Hình ảnh phòng ${currentModalImageIndex + 1}`;
        console.log(`Đã cập nhật modal image src thành: ${imageUrl}`); // DEBUG
    } else {
         console.warn("Không có ảnh để hiển thị trong modal hoặc index bị sai.");
         // Có thể hiển thị ảnh mặc định
         modalMainImage.src = '../assets/img/room-default.jpg';
         modalMainImage.alt = 'Ảnh phòng mặc định';
    }
}
