    ...
    try {
        ...
        if (result.hoaDonId) {
            try {
                ...
                const response = await fetch(`/api/Hoadons/${result.hoaDonId}`);
                const hoaDonData = await response.json();
                
                console.log('📄 Dữ liệu hóa đơn:', hoaDonData);
                
                // CHỈ LẤY TIỀN PHÒNG (đã giảm giá đoàn + điểm), KHÔNG LẤY TIỀN DỊCH VỤ
                const tienPhongHoaDon = hoaDonData.tienphong || hoaDonData.TienPhong || 0;
                finalAmount = tienPhongHoaDon;
                
                console.log('💰 Tiền phòng từ hóa đơn (truyền VNPay):', formatPrice(finalAmount));
                console.log('ℹ️ Tiền dịch vụ sẽ trả riêng tại khách sạn');
                
            } catch (error) {
                console.error('❌ Lỗi lấy hóa đơn:', error);
                // Fallback: tính tiền phòng từ frontend
                ...
                console.warn('⚠️ Fallback về tính toán tiền phòng frontend:', formatPrice(finalAmount));
            }
        } else {
