...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        console.log(' DEBUG TÍNH TỔNG:');
        console.log('- result.tongTien (tiền phòng):', result.tongTien);
        
        // Tính tiền dịch vụ từ bookingData
        let totalServicePrice = 0;
        if (bookingData.rooms) {
            bookingData.rooms.forEach(room => {
                if (room.services) {
                    const servicePrice = room.services.reduce((sum, s) => sum + (s.dongia * s.soluong), 0);
                    totalServicePrice += servicePrice;
                }
            });
        }
        
        console.log('- Tiền dịch vụ:', totalServicePrice);
        
        // Tổng = tiền phòng + tiền dịch vụ
        const finalAmount = (result.tongTien || 0) + totalServicePrice;
        
        console.log(' Tổng cuối cùng (phòng + dịch vụ):', finalAmount);
        console.log(' Khớp với 499000?', finalAmount === 499000);
        ...
        // ⏳ ĐỢI 10 GIÂY ĐỂ KIỂM TRA LOGS
        console.log('⏳⏳⏳ ĐỢI 10 GIÂY ĐỂ BẠN KIỂM TRA LOGS ⏳⏳⏳');
        console.log('📋 Hãy kiểm tra Console để xem logs về dịch vụ!');
        console.log('🔍 Mở tab Network để xem API calls đến /Sudungdvs/sudungdv!');
        await new Promise(r => setTimeout(r, 1000)); // Đợi 10 giây
        console.log('✅ Hết thời gian đợi, chuyển sang VNPay...');
...
function showGroupBookingModal(totalRooms, totalPrice) {
    ...
    // Tách riêng tiền phòng và tiền dịch vụ
    const bookingData = JSON.parse(localStorage.getItem('currentBooking') || '{}');
    ...
    if (bookingData.rooms) {
        bookingData.rooms.forEach(room => {
            // CHỈ TIỀN PHÒNG
            roomOnlyPrice += room.price * (room.quantity || 1) * nights;
            
            // TIỀN DỊCH VỤ (không giảm)
            if (room.services) {
    ...
    // CHỈ GIẢM GIÁ TIỀN PHÒNG
    const roomDiscount = roomOnlyPrice * discountPercent / 100;
    const finalRoomPrice = roomOnlyPrice - roomDiscount;
    const finalTotalPrice = finalRoomPrice + servicePrice; // Dịch vụ không đổi
