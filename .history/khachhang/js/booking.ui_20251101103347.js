...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        // **CHỈ LẤY TIỀN PHÒNG TỪ HÓA ĐƠN (KHÔNG BAO GỒM DỊCH VỤ)**
        let finalAmount = 0;
        
        if (result.hoaDonId) {
            try {
                console.log('🔍 Lấy hóa đơn theo ID:', result.hoaDonId);
                
                // Gọi API lấy hóa đơn chi tiết
                const response = await fetch(`https://localhost:7076/api/Hoadons/${result.hoaDonId}`);
                const hoaDonData = await response.json();
                
                console.log('📄 Dữ liệu hóa đơn:', hoaDonData);
                console.log('📄 Các field có sẵn:', Object.keys(hoaDonData));
                
                // LẤY TỔNG TIỀN HÓA ĐƠN (thử nhiều field có thể)
                const rawAmount = hoaDonData.tongcong || hoaDonData.TongCong || 
                                 hoaDonData.tongtien || hoaDonData.TongTien ||
                                 hoaDonData.tongTien || 0;
                
                // ĐẢM BẢO LÀ SỐ NGUYÊN CHÍNH XÁC
                finalAmount = Math.round(parseFloat(rawAmount));
                             
                console.log('💰 Field được sử dụng:', {
                    tongcong: hoaDonData.tongcong,
                    TongCong: hoaDonData.TongCong,
                ...
                console.log('🔍 KIỂM TRA CHI TIẾT SỐ TIỀN:');
                console.log('- Type của finalAmount:', typeof finalAmount);
                console.log('- Raw value:', finalAmount);
                console.log('- Parsed int:', parseInt(finalAmount));
                console.log('- Math.round:', Math.round(finalAmount));
                console.log('- Số tiền format:', formatPrice(finalAmount));
                
                console.log('💰 Tổng tiền hóa đơn (phòng + dịch vụ) truyền VNPay:', formatPrice(finalAmount));
                
            } catch (error) {
                console.error('❌ Lỗi lấy hóa đơn:', error);
                // Fallback về result.tongTien nếu API thất bại
                finalAmount = result.tongTien || 0;
                console.warn('⚠️ Fallback về result.tongTien:', formatPrice(finalAmount));
            }
        } else {
            console.error('❌ Không có hoaDonId từ result');
            finalAmount = 0;
        }
        ...
        console.log('🚀 TRUYỀN VÀO VNPAY:');
        console.log('- Hóa đơn ID:', result.hoaDonId);
        console.log('- Số tiền gửi:', finalAmount);
        console.log('- Type:', typeof finalAmount);
        console.log('- Là số nguyên?', Number.isInteger(finalAmount));
        ...
        // Thêm timeout fallback nếu redirect không thành công
        setTimeout(() => {
...
function showGroupBookingModal(totalRooms, totalPrice) {
    ...
    // Tính tiền phòng và tiền dịch vụ riêng biệt
    const bookingData = JSON.parse(localStorage.getItem('currentBooking') || '{}');
    const nights = bookingData.searchInfo?.nights || 1;
    ...
    if (bookingData.rooms) {
        bookingData.rooms.forEach(room => {
            // Tiền phòng
            roomPrice += room.price * (room.quantity || 1) * nights;
            
            // Tiền dịch vụ
            if (room.services) {
                servicePrice += room.services.reduce((sum, s) => sum + (s.dongia * s.soluong), 0);
            }
        });
    }
    
    // CHỈ GIẢM GIÁ TIỀN PHÒNG, KHÔNG GIẢM DỊCH VỤ
    const roomDiscount = roomPrice * discountPercent / 100;
    const finalRoomPrice = roomPrice - roomDiscount;
    const finalTotalPrice = finalRoomPrice + servicePrice; // Dịch vụ giữ nguyên

    document.getElementById('modalRoomCount').textContent = `${totalRooms} phòng`;
    document.getElementById('modalDiscount').textContent = 
        discountPercent > 0 ? `${discountPercent}% chỉ áp dụng tiền phòng (${formatPrice(roomDiscount)} VNĐ)` : 'Không áp dụng';
    document.getElementById('modalTotal').textContent = formatPrice(finalTotalPrice) + ' VNĐ';
