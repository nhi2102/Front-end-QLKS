...
function showPointsDiscount(discountAmount, pointsUsed) {
    ...
    console.log(`💰 Hiển thị giảm giá: ${discountAmount.toLocaleString('vi-VN')} VNĐ từ ${pointsUsed.toLocaleString('vi-VN')} điểm`);
}
...
function updateTotalPrice() {
    ...
    console.log(`   - Tổng ban đầu (phòng + dịch vụ): ${originalTotal.toLocaleString('vi-VN')} VNĐ`);
    console.log(`   - Điểm sử dụng: ${pointsUsed.toLocaleString('vi-VN')} điểm`);
    console.log(`   - Giảm giá: ${discountAmount.toLocaleString('vi-VN')} VNĐ`);
    console.log(`   - Tổng cuối cùng: ${finalTotal.toLocaleString('vi-VN')} VNĐ`);
}
...
function debugBookingState() {
    ...
    if (bookingData.rooms) {
        bookingData.rooms.forEach((room, idx) => {
            ...
            console.log(`   Phòng ${idx + 1}: ${room.name || room.title}`);
            console.log(`      - Giá phòng: ${(room.price || 0).toLocaleString('vi-VN')} VNĐ`);
            console.log(`      - Dịch vụ: ${servicesTotal.toLocaleString('vi-VN')} VNĐ`);
        });
    }
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        // Nếu muốn thanh toán đầy đủ (không dùng điểm), đổi thành originalTotal
        ...
        const finalAmount = frontendFinalAmount;
        
        console.log(`💰 Chi tiết tính toán giá:`);
        console.log(`   - Tổng hóa đơn (phòng + dịch vụ): ${originalTotal.toLocaleString('vi-VN')} VNĐ`);
        console.log(`   - Điểm sử dụng: ${pointsUsed.toLocaleString('vi-VN')} điểm`);
        console.log(`   - Frontend tính (sau trừ điểm): ${frontendFinalAmount.toLocaleString('vi-VN')} VNĐ`);
        console.log(`   - Backend trả về: ${result.tongTien?.toLocaleString('vi-VN')} VNĐ`);
        console.log(`   - 🎯 SẼ THANH TOÁN: ${finalAmount.toLocaleString('vi-VN')} VNĐ`);
        
        // Log thêm thông tin debug
        console.log(`🔍 Debug thanh toán:`);
        console.log(`   - Booking ID: ${result.bookingId}`);
        console.log(`   - Hóa đơn ID: ${result.hoaDonId}`);
        console.log(`   - Điểm có được trừ không: ${pointsUsed > 0 ? 'CÓ' : 'KHÔNG'}`);
        ...
        setTimeout(() => {
            console.warn('⚠️ Redirect có thể bị chặn, thử mở trong tab mới');
