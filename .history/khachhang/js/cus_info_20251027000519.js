...
async function displayLoyaltyPoints(bookingData) {
    ...
    try {
        ...
        if (!makhachhang) {
            console.warn('⚠️ Không tìm thấy mã khách hàng để lấy điểm tích lũy');
            return;
        }
...
function displayHotelInfo(bookingData) {
    ...
    // Nếu có dateRange thì tính chính xác số ngày / đêm
    if (searchInfo.dateRange && searchInfo.dateRange.includes(' - ')) {
        const [startStr, endStr] = searchInfo.dateRange.split(' - ').map(s => s.trim());
        ...
        nightsText = `${days} ngày ${nights} đêm`;
    } else {
        nightsText = '1 ngày 1 đêm';
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            try {
                ...
                console.error(' Lỗi khi cập nhật điểm tích lũy (không ảnh hưởng đặt phòng):', pointsError);
            }
            ...
            if (result.isGroupBooking && result.totalRooms > 5) {
                ...
                showGroupBookingModal(result.totalRooms, discountPercent, giamGia, tongTien);
            }
            ...
            if (!tongTien || tongTien === 0) {
                ...
                console.warn(' tongTien = 0/null, tính lại từ donGia - giamGia:', { donGia, giamGia, tongTien });
            }
            ...
            if (paymentMethod === 'vnpay') {
                ...
                try {
                    ...
                    console.log(' Gọi createVNPayPayment với params:', {
                        ...
                        returnUrl: `${window.location.origin}/../khachhang/payment-result.html`
                    });
                    ...
                    setTimeout(() => window.location.href = '../khachhang/home.html', 3000);
                    return;
                }
            }
            ...
            setTimeout(() => window.location.href = '../khachhang/my_room.html', 2000);
        } else {
...
function showGroupBookingModal(totalRooms, discountPercent, giamGia, tongTien) {
    const modal = document.getElementById('groupBookingModal');
    if (!modal) {
        console.error('Không tìm thấy modal groupBookingModal');
        return;
    }
    ...
    document.getElementById('modalDiscount').textContent = 
        `${discountPercent}% (${giamGia.toLocaleString('vi-VN')} VNĐ)`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modal) {
        ...
        if (overlay) {
            overlay.addEventListener('click', closeGroupBookingModal);
        }
    }
});
