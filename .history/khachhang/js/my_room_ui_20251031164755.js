...
document.addEventListener('DOMContentLoaded', async function() {
            ...
            async function getTongTienFromHoaDon(hoaDonId) {
                if (!hoaDonId) {
                    console.warn('⚠️ getTongTienFromHoaDon: Không có hoaDonId');
                    return 0;
                }

                try {
                    const apiUrl = `https://localhost:7076/api/Hoadons/${hoaDonId}`;
                    console.log(`🧾 Bắt đầu gọi API hóa đơn: ${apiUrl}`);
                    ...
                    console.log(`📡 Response nhận được sau ${endTime - startTime}ms:`);
                    console.log(`   - Status: ${response.status} ${response.statusText}`);
                    console.log(`   - URL: ${apiUrl}`);

                    if (!response.ok) {
                        console.warn(`❌ API hóa đơn thất bại: ${response.status} - ${response.statusText}`);
                        
                        // Thử đọc response text để xem lỗi gì
                        try {
                            const errorText = await response.text();
                            console.warn(`❌ Error details: ${errorText}`);
                        } catch (e) {
                            console.warn(`❌ Không thể đọc error details`);
                        }
                    ...
                    console.log(`✅ API Response thành công cho hóa đơn ${hoaDonId}`);

                    // DEBUG: In toàn bộ dữ liệu hóa đơn
                    console.log(`🔍 DEBUG - Raw hóa đơn data cho ID ${hoaDonId}:`, hoaDon);
                    ...
                    console.log(`🔍 DEBUG - Tất cả field tiền cho ID ${hoaDonId}:`, tongTienCandidates);
                    ...
                    console.log(`💰 KẾT QUÀ - Hóa đơn ${hoaDonId}: ${tongTien > 0 ? tongTien.toLocaleString('vi-VN') + ' VNĐ' : 'KHÔNG CÓ DỮ LIỆU'}`);
                    
                    if (tongTien === 0) {
                        console.warn(`⚠️ Hóa đơn ${hoaDonId} không có tổng tiền hoặc = 0`);
                        console.warn(`🔍 Các field đã kiểm tra:`, Object.keys(tongTienCandidates));
                    }
                    
                    return tongTien;

                } catch (error) {
                    console.error(`❌ Exception khi gọi API hóa đơn ${hoaDonId}:`, error);
            ...
            async function createBookingCard(booking, isUpcoming) {
                ...
                console.log(`🎯 Booking #${madatphong} - Tìm hóa đơn với ID: ${hoaDonId}`);

                if (hoaDonId) {
                    console.log(`📞 Gọi API hóa đơn cho booking #${madatphong}...`);
                    tongTienHoaDon = await getTongTienFromHoaDon(hoaDonId);
                    
                    console.log(`🔍 KẾT QUẢ API HÓA ĐƠN cho booking #${madatphong}:`);
                    console.log(`   - Hóa đơn ID: ${hoaDonId}`);
                    console.log(`   - Tổng tiền trả về: ${tongTienHoaDon}`);
                    console.log(`   - Có dữ liệu hóa đơn: ${tongTienHoaDon > 0 ? 'CÓ' : 'KHÔNG'}`);
                    
                    if (tongTienHoaDon > 0) {
                        console.log(`✅ API HÓA ĐƠN THÀNH CÔNG - Booking #${madatphong}: ${tongTienHoaDon.toLocaleString('vi-VN')} VNĐ`);
                    } else {
                        console.warn(`⚠️ API HÓA ĐƠN TRỐNG - Booking #${madatphong}: Sẽ fallback sang dữ liệu khác`);
                    }
                } else {
                    console.warn('⚠️ Không có mã đặt phòng để tìm hóa đơn');
                }
                ...
                console.log('📊 TỔNG KẾT TỪ CÁC NGUỒN KHÁC NHAU:');
                console.log('   - Mã đặt phòng/hóa đơn:', hoaDonId);
                console.log('   - 🏷️ DONGIA (trước giảm):', donGia?.toLocaleString('vi-VN') || 0);
                console.log('   - 🏷️ GIAMGIA:', giamGia?.toLocaleString('vi-VN') || 0);
                console.log('   - 🏠 DATPHONG.TONGTIEN:', tongTienPhongTuDatPhong?.toLocaleString('vi-VN') || 0);
                console.log('   - 🧾 API HOADON.TONGTIEN:', tongTienHoaDon?.toLocaleString('vi-VN') || 0);
                console.log('   - 💰 Tiền phòng (sau giảm):', tongTienPhongAll?.toLocaleString('vi-VN') || 0);
                console.log('   - 🛎️ Tiền dịch vụ:', tongTienDichVuAll?.toLocaleString('vi-VN') || 0);
                console.log('   - 📄 BOOKING.tongtien gốc:', (booking.tongtien || booking.Tongtien)?.toLocaleString('vi-VN') || 0);
                ...
                chiTiet.forEach((ct, index) => {
                            ...
                            console.log(`🏨 DEBUG phòng ${index + 1}:`);
                            console.log('   - phongInfo:', phongInfo);
                            console.log('   - loaiphongInfo:', loaiphongInfo);
                            console.log('   - ct (chi tiết gốc):', ct);
                            console.log(`   - tongCong từ DB: ${tongCong} (field: ${ct.tongcong ? 'tongcong' : 'Tongcong'})`);
                            console.log('   - giaPhong ban đầu:', giaPhong);
                            ...
                            console.log('🛎️ Tổng tiền dịch vụ phòng này:', tongTienDichVu.toLocaleString('vi-VN'));

                            console.log('🔍 DEBUG - Tính tiền phòng:');
                            console.log(`   - tongCong (từ DB): ${tongCong.toLocaleString('vi-VN')} VNĐ`);
                            console.log(`   - tongTienDichVu: ${tongTienDichVu.toLocaleString('vi-VN')} VNĐ`);
                            console.log(`   - nights: ${nights} đêm`);

                            // Tính tiền phòng từ TỔNG TIỀN ĐÃ GIẢM (không dùng giá gốc)
                            // tongCong = tiền phòng + tiền dịch vụ của phòng này
                            const tienPhong = tongCong - tongTienDichVu;

                            console.log(`   - tienPhong = tongCong - tongTienDichVu = ${tienPhong.toLocaleString('vi-VN')} VNĐ`);

                            // Tính lại giaPhong từ tienPhong để hiển thị (đã bao gồm giảm giá theo đoàn)
                            let giaPhongTinhToan = 0;
                            if (nights > 0) {
                                giaPhongTinhToan = Math.round(tienPhong / nights);
                                giaPhong = giaPhongTinhToan;
                            }

                            console.log(`   - giaPhong = tienPhong / nights = ${giaPhongTinhToan.toLocaleString('vi-VN')} VNĐ/đêm`);
                            console.log('💰 KẾT QUẢ - Giá phòng/đêm (đã giảm):', giaPhong.toLocaleString('vi-VN'));
                            console.log('💰 KẾT QUẢ - Tổng tiền phòng (đã giảm):', tienPhong.toLocaleString('vi-VN'));
        ...
        // **ƯU TIÊN TỔNG TIỀN TỪ HÓA ĐƠN - LUÔN LUÔN**
        let tongTienCuoiCung;
        
        if (tongTienHoaDon > 0) {
            // LUÔN SỬ DỤNG HÓA ĐƠN NẾU CÓ - KHÔNG FALLBACK
            tongTienCuoiCung = tongTienHoaDon;
            console.log(`✅ SỬ DỤNG TỔNG TIỀN TỪ HÓA ĐƠN: ${tongTienHoaDon.toLocaleString('vi-VN')} VNĐ`);
            
            // Cập nhật lại breakdown theo tỷ lệ từ hóa đơn
            const tongTinhToan = tongTienPhongAll + tongTienDichVuAll;
            if (tongTinhToan > 0 && Math.abs(tongTinhToan - tongTienHoaDon) > 1000) {
                console.log(`📊 Điều chỉnh breakdown theo hóa đơn:`);
                console.log(`   - Tổng tính toán: ${tongTinhToan.toLocaleString('vi-VN')}`);
                console.log(`   - Tổng hóa đơn: ${tongTienHoaDon.toLocaleString('vi-VN')}`);
                
                // Tính tỷ lệ phòng và dịch vụ
                const tylePhong = tongTienPhongAll / tongTinhToan;
                const tyleDichVu = tongTienDichVuAll / tongTinhToan;
                
                tongTienPhongAll = Math.round(tongTienHoaDon * tylePhong);
                tongTienDichVuAll = Math.round(tongTienHoaDon * tyleDichVu);
                
                console.log(`   - Tiền phòng điều chỉnh: ${tongTienPhongAll.toLocaleString('vi-VN')}`);
                console.log(`   - Tiền dịch vụ điều chỉnh: ${tongTienDichVuAll.toLocaleString('vi-VN')}`);
            }
        } else {
            // Fallback: chỉ khi KHÔNG có hóa đơn
            const tongTinhToan = tongTienPhongAll + tongTienDichVuAll;
            
            if (tongTinhToan > 0 && (tongTien === 0 || Math.abs(tongTinhToan - tongTien) > 1000)) {
                console.log(`⚠️ Fallback - Không có hóa đơn, dùng tính toán: ${tongTinhToan.toLocaleString('vi-VN')} VNĐ`);
                tongTienCuoiCung = tongTinhToan;
            } else {
                console.log(`⚠️ Fallback - Không có hóa đơn, dùng DATPHONG: ${tongTien.toLocaleString('vi-VN')} VNĐ`);
                tongTienCuoiCung = tongTien;
            }
        }

        
        console.log(`🎯 ===== TỔNG KẾT CUỐI CÙNG cho Booking #${madatphong} =====`);
        console.log(`   - 🧾 Tổng từ HÓA ĐƠN API: ${tongTienHoaDon?.toLocaleString('vi-VN') || 0} VNĐ`);
        console.log(`   - 🏠 Tổng từ DATPHONG: ${tongTien?.toLocaleString('vi-VN') || 0} VNĐ`);
        console.log(`   - 💰 Tiền phòng: ${tongTienPhongAll?.toLocaleString('vi-VN') || 0} VNĐ`);
        console.log(`   - 🛎️ Tiền dịch vụ: ${tongTienDichVuAll?.toLocaleString('vi-VN') || 0} VNĐ`);
        console.log(`   - 📊 Nguồn được chọn: ${tongTienHoaDon > 0 ? '🧾 HÓA ĐƠN API' : '� TÍNH TOÁN/DATPHONG'}`);
        console.log(`   - �🎯 TỔNG CUỐI CÙNG: ${tongTienCuoiCung?.toLocaleString('vi-VN') || 0} VNĐ`);
        
        if (tongTienHoaDon > 0 && tongTienCuoiCung !== tongTienHoaDon) {
            console.error(`❌ LỖI LOGIC: Có hóa đơn (${tongTienHoaDon}) nhưng tổng cuối = ${tongTienCuoiCung}`);
        }
        
        if (madatphong == 50) {
            console.log(`🔍 ĐẶC BIỆT - Debug booking #50:`);
            console.log(`   - tongTienHoaDon: ${tongTienHoaDon}`);
            console.log(`   - tongTienCuoiCung: ${tongTienCuoiCung}`);
            console.log(`   - Có sử dụng hóa đơn: ${tongTienHoaDon > 0}`);
        }
