...
document.addEventListener('DOMContentLoaded', async function() {
            ...
            async function createBookingCard(booking, isUpcoming) {
                ...
                const hoaDonId = booking.mahoadon || booking.MaHoaDon || booking.hoadonId || 
                               booking.HoaDonId || booking.hoaDonId || booking.maHoaDon;
                let tongTienHoaDon = 0;
                
                console.log(`🎯 Mã hóa đơn sẽ dùng: ${hoaDonId}`);
                
                if (hoaDonId) {
                    tongTienHoaDon = await getTongTienFromHoaDon(hoaDonId);
                } else {
                    console.warn('⚠️ Không tìm thấy mã hóa đơn trong booking data');
                }
