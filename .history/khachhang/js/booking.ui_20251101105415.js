...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        // **LẤY TỔNG TIỀN TỪ HÓA ĐƠN (không phải từ đặt phòng)**
        let finalAmount = 0;
        
        if (result.hoaDonId) {
            try {
                console.log('🔍 Lấy tổng tiền từ hóa đơn ID:', result.hoaDonId);
                
                const response = await fetch(`https://localhost:7076/api/Hoadons/${result.hoaDonId}`);
                const hoaDonData = await response.json();
                
                console.log('📄 TOÀN BỘ dữ liệu hóa đơn:', JSON.stringify(hoaDonData, null, 2));
                console.log('📄 Các field có sẵn:', Object.keys(hoaDonData));
                
                // Thử tất cả field có thể chứa tổng tiền
                console.log('🔍 TẤT CẢ FIELD SỐ TIỀN:');
                console.log('- tongtien:', hoaDonData.tongtien);
                console.log('- TongTien:', hoaDonData.TongTien);
                console.log('- tongTien:', hoaDonData.tongTien);
                console.log('- tongcong:', hoaDonData.tongcong);
                console.log('- TongCong:', hoaDonData.TongCong);
                console.log('- total:', hoaDonData.total);
                console.log('- amount:', hoaDonData.amount);
                
                // LẤY NGUYÊN XI (thử tất cả field)
                finalAmount = hoaDonData.tongtien || hoaDonData.TongTien || hoaDonData.tongTien || 
                             hoaDonData.tongcong || hoaDonData.TongCong || 0;
                
                console.log('💰 Final amount lấy được:', finalAmount, typeof finalAmount);
        ...
        const vnpayUrl = await BookingAPI.createVNPayURL({
            ...
            amount:  parseInt(finalAmount, 10),  // đã là giá sau giảm
