...
async function loadUserInfo() {
    try {
        ...
        if (localUser.email) fillFormData(localUser);
    }
}
...
async function displayLoyaltyPoints(data) {
    try {
        const customer = await BookingAPI.getCustomerInfo();
        const currentPoints = customer.diemthanhvien || customer.Diemthanhvien || 0;
...
function showGroupBookingModal(totalRooms, totalPrice) {
    const discountPercent = totalRooms > 10 ? 10 : totalRooms > 7 ? 5 : 3;
    ...
    document.getElementById('modalDiscount').textContent = 
        `${discountPercent}% (${formatPrice(discount)} VNĐ)`;
    ...
    // ✅ Thêm tự động đóng modal sau 3 giây
    setTimeout(() => {
