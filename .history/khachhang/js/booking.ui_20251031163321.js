...
// Biến để track event handlers đã setup
...
function setupEventHandlers() {
    ...
    if (eventHandlersSetup) {
        console.warn('⚠️ Event handlers đã được setup, bỏ qua...');
        return;
    }
    ...
    if (form) {
        ...
        console.log('✅ Form submit listener đã setup');
    }
...
async function handleFormSubmit(e) {
    ...
    if (isSubmitting) {
        console.warn('⚠️ Đang xử lý booking, vui lòng đợi...');
        alert('Đang xử lý đặt phòng, vui lòng đợi...');
        return;
    }

    // Check if already submitted recently (within 10 seconds)
    const lastSubmit = localStorage.getItem('lastBookingSubmit');
    const now = Date.now();
    if (lastSubmit && (now - parseInt(lastSubmit)) < 10000) {
        console.warn('⚠️ Submit quá nhanh, có thể duplicate!');
        alert('Vui lòng đợi ít nhất 10 giây trước khi đặt phòng lại');
        return;
    }

    isSubmitting = true;
    localStorage.setItem('lastBookingSubmit', now.toString());
    ...
    try {
        ...
        console.log('📋 Booking data trước khi gửi:', bookingData);
        console.log('💰 Tổng tiền:', bookingData.totalPrice);
        console.log('🎯 Điểm sử dụng:', bookingData.diemSuDung || 0);
        
        // Thêm unique identifier để tránh duplicate
        const timestamp = Date.now();
        bookingData.clientTimestamp = timestamp;
        ...
        console.log("🔢 Booking ID:", result.bookingId || result.maDatPhong);
        console.log("🧾 Invoice ID:", result.hoaDonId || result.maHoaDon);
