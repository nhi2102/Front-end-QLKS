...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        // **LẤY SỐ TIỀN CHÍNH XÁC TỪ HÓA ĐƠN API**
        console.log('🔍 DEBUG PAYMENT AMOUNT:');
        console.log('- result.tongTien từ API:', result.tongTien);
        console.log('- result.hoaDonId:', result.hoaDonId);
        console.log('- frontendFinalAmount:', frontendFinalAmount);
        
        let finalAmount;
        
        if (result.hoaDonId && result.tongTien !== undefined) {
            // Ưu tiên số tiền từ hóa đơn đã được tạo (đã tính giảm giá đoàn)
            finalAmount = result.tongTien;
            console.log('✅ Sử dụng số tiền từ hóa đơn API:', formatPrice(finalAmount));
        } else {
            // Fallback nếu API không trả về đúng
            finalAmount = frontendFinalAmount;
            console.warn('⚠️ Fallback về tính toán frontend:', formatPrice(finalAmount));
        }
