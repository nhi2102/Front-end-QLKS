...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function initializeAmenitiesModal() {
        ...
        async function openAmenitiesModal(roomTypeId) { // Đổi tên tham số cho rõ
    // Lấy roomTypeId (BỎ tiền tố nếu có)
    // Ví dụ: Nếu data-room-type="roomtype-2", thì roomId = "2"
    // Nếu data-room-type="2", thì roomId = "2"
    ...
    try {
        ...
        const roomsData = window.roomsData || []; // Đảm bảo đây là mảng loại phòng bạn thấy trong console

        // ▼▼▼ SỬA LOGIC TÌM KIẾM Ở ĐÂY ▼▼▼
        // Tìm loại phòng dựa trên MALOAIPHONG
        const roomTypeInfo = roomsData.find(rt => rt.maloaiphong == targetMaLoaiPhong); 
        // ▲▲▲ KẾT THÚC SỬA ▲▲▲

        if (roomTypeInfo) {
            // Cập nhật thông tin modal DỰA TRÊN LOẠI PHÒNG
            document.getElementById('modalRoomTitle').textContent = roomTypeInfo.tenloaiphong || 'Loại Phòng';
            document.getElementById('modalRoomSize').textContent = roomTypeInfo.songuoitoida ?
                `${roomTypeInfo.songuoitoida} người` : 'N/A';
            document.getElementById('modalRoomDescription').textContent = roomTypeInfo.mota || 'Mô tả đang cập nhật...';

            // Set images (Lấy từ thông tin hình ảnh của loại phòng nếu có)
            // Giả sử API trả về mảng hinhAnhPhu trong loại phòng
            let images = ['../assets/img/room-default.jpg']; // Mặc định
            if (roomTypeInfo.hinhAnhPhongs && roomTypeInfo.hinhAnhPhongs.length > 0) {
                 images = roomTypeInfo.hinhAnhPhongs.map(img => img.duongDan); // Lấy đường dẫn ảnh
            } else if (roomTypeInfo.hinhAnh) { // Hoặc lấy từ trường hinhanh chính
                 images = [roomTypeInfo.hinhAnh];
            }
