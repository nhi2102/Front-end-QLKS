...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function showCancelModal(booking) {
        ...
        // Lấy TONGTIEN từ DATPHONG (đã được trigger tính toán bao gồm giảm giá)
        let tongTien = booking.tongtien || booking.Tongtien || 0;
        
        // Nếu TONGTIEN = 0 hoặc null, dùng DONGIA (giá trước giảm)
        if (tongTien === 0 || tongTien === null) {
            const donGia = booking.dongia || booking.Dongia || 0;
            const giamGia = booking.giamgia || booking.Giamgia || 0;
            tongTien = donGia - giamGia;
            
            // Nếu vẫn = 0, thử tính từ chiTiet (fallback cuối cùng)
            if (tongTien === 0) {
                const chiTiet = booking.chiTiet || [];
                chiTiet.forEach(ct => {
            ...
            console.log(`📊 Modal hủy - Tổng tiền: ${tongTien} (dongia: ${donGia}, giamgia: ${giamGia})`);
        }
