...
async function getMyBookings() {
    try {
        ...
        if (!makhachhang) {
            console.log('getMyBookings: currentUser found but no explicit id field; will try fallback matching (phone/email)');
        } else {
        ...
        // Lọc theo mã khách hàng (so sánh dưới dạng string, hỗ trợ nhiều tên trường)
        const normalizeId = (v) => (v === undefined || v === null) ? '' : String(v).trim();
        const targetId = normalizeId(makhachhang);

        const myDatPhongs = allDatPhongs.filter(dp => {
            const dpIds = [dp.makh, dp.Makh, dp.makhachhang, dp.Makhachhang, dp.makh_kh, dp.makh_khach];
            for (const cand of dpIds) {
                if (normalizeId(cand) === targetId && targetId !== '') return true;
            }
            return false;
        });

        console.log(`Tìm thấy ${myDatPhongs.length} đặt phòng (match by id)`);

        // If nothing found by id, try fallback matching by phone/email or nested navigation
        if ((!myDatPhongs || myDatPhongs.length === 0)) {
            try {
                const normalizePhone = s => (s || '').toString().replace(/[^\d]/g, '');
                const userPhones = [currentUser.sdt, currentUser.Sdt, currentUser.phone, currentUser.sodt, currentUser.SDT].map(x => normalizePhone(x)).filter(x=>x);
                const userEmails = [currentUser.email, currentUser.Email].map(x => (x||'').toString().toLowerCase()).filter(x=>x);

                const altMatches = allDatPhongs.filter(dp => {
                    // check common phone/email fields on booking or nested customer
                    const candPhones = [dp.soDienThoai, dp.Sdt, dp.sdt, dp.sodt, (dp.makhNavigation && (dp.makhNavigation.Sdt || dp.makhNavigation.sdt))];
                    const candEmails = [dp.email, dp.Email, (dp.makhNavigation && (dp.makhNavigation.Email || dp.makhNavigation.email))];

                    for (const up of userPhones) {
                        if (!up) continue;
                        for (const cp of candPhones) {
                            if (!cp) continue;
                            if (normalizePhone(cp) === up) return true;
                        }
                    }
                    ...
                    for (const ue of userEmails) {
                        ...
                        for (const ce of candEmails) {
                            if (!ce) continue;
                            if ((ce||'').toString().toLowerCase() === ue) return true;
                        }
                    }

                    // also check nested booking customer id fields
                    const nestedId = dp.makhNavigation && (dp.makhNavigation.makh || dp.makhNavigation.MaKh || dp.makhNavigation.id);
                    if (nestedId && String(nestedId).trim() === String(makhachhang).trim()) return true;

                    return false;
                });

                if (altMatches && altMatches.length > 0) {
                    console.log(`Tìm thấy ${altMatches.length} đặt phòng bằng fallback (phone/email)`);
                    // replace myDatPhongs with altMatches
                    myDatPhongs.splice(0, myDatPhongs.length, ...altMatches);
                } else {
                    console.log('Không tìm thấy đặt phòng bằng fallback matching');
                }
            } catch (err) {
                console.error('Lỗi khi thử fallback matching:', err);
            }
        }
