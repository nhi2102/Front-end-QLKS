    ...
    try {
        ...
        container.addEventListener("click", async(e) => {
            if (e.target.classList.contains("room-title")) {
                ...
                try {
                    const resImg = await fetch(`https://localhost:7076/api/HinhPhongs/ByLoaiPhong/${roomId}`);
                    if (!resImg.ok) throw new Error("Không tải được ảnh phụ!");
                    const imgData = await resImg.json();

                    // 🔍 Lấy danh sách tất cả hình phụ không null
                    const imageList = [
                        imgData.hinhphu1,
                        imgData.hinhphu2,
                        imgData.hinhphu3,
                    ...
                    if (imageList.length === 0) {
                        ...
                        gallery.innerHTML = imageList.map(img =>
                            `<img src="/assets/img/${img}" alt="Ảnh phụ" class="sub-image">`
                        ).join("");
                    }
