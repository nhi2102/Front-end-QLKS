...
async function getMyBookings() {
    try {
        ...
        // Debug: In ra toàn bộ currentUser để kiểm tra
        console.log('🔍 Debug currentUser từ localStorage:', currentUser);

        // Kiểm tra đã đăng nhập chưa (chỉ cần có email hoặc phone)
        if (!currentUser || (!currentUser.email && !currentUser.sdt && !currentUser.Email && !currentUser.Sdt && !currentUser.phone && !currentUser.Phone)) {
            throw new Error('Vui lòng đăng nhập để xem đặt phòng!');
        }

        // Thử tất cả các cách có thể để lấy mã khách hàng
        ...
        console.log('🔍 Mã khách hàng tìm được:', makhachhang);
        console.log('📧 Email:', currentUser.email || currentUser.Email);
        console.log('📱 SĐT:', currentUser.sdt || currentUser.Sdt || currentUser.phone || currentUser.Phone);
        console.log('🔍 Tất cả keys trong currentUser:', Object.keys(currentUser));
...
async function checkReviewStatus(madatphong) {
    try {
        ...
        console.log('🔍 Mã khách hàng cho check review:', makhachhang);
...
async function submitReview(reviewData) {
    try {
        ...
        // Nếu không tìm thấy mã khách hàng trong localStorage, thử lấy từ đặt phòng
        let finalMaKH = makhachhang;
        
        if (!finalMaKH) {
            console.log('❌ Không tìm thấy mã khách hàng trong localStorage, đang tìm từ đặt phòng...');
            
            try {
                // Lấy mã khách hàng từ đặt phòng hiện tại
                const bookingDetails = await getBookingDetails(reviewData.madatphong);
                if (bookingDetails && bookingDetails.length > 0) {
                    // Lấy thông tin đặt phòng từ API để tìm mã khách hàng
                    const dpResponse = await fetch(`${API_BASE}/Datphongs`);
                    if (dpResponse.ok) {
                        const allDatPhongs = await dpResponse.json();
                        const currentDP = allDatPhongs.find(dp => 
                            (dp.madatphong === reviewData.madatphong || dp.Madatphong === reviewData.madatphong)
                        );
                        
                        if (currentDP) {
                            finalMaKH = currentDP.makh || currentDP.Makh;
                            console.log('✅ Tìm thấy mã khách hàng từ đặt phòng:', finalMaKH);
                        }
                    }
                }
            } catch (error) {
                console.error('Lỗi khi tìm mã khách hàng từ đặt phòng:', error);
            }
        }

        if (!finalMaKH) {
            const userEmail = currentUser.email || currentUser.Email;
            const userPhone = currentUser.sdt || currentUser.Sdt || currentUser.phone || currentUser.Phone;
            
            console.log('❌ Không tìm thấy mã khách hàng');
            console.log('📧 Email:', userEmail);
            console.log('📱 Số điện thoại:', userPhone);
            
            throw new Error(`Không tìm thấy mã khách hàng! Vui lòng đăng xuất và đăng nhập lại.\nThông tin hiện tại: Email=${userEmail}, SĐT=${userPhone}`);
        }
        
        // Sử dụng finalMaKH thay vì makhachhang
        const makhachhang_final = finalMaKH;
        
        console.log('✅ Mã khách hàng cuối cùng sử dụng:', makhachhang_final);
...
/**
 * Debug function - Kiểm tra thông tin đăng nhập hiện tại
 */
function debugCurrentUser() {
    ...
    console.log('🔑 Các keys có sẵn:', Object.keys(currentUser));
    
    // Kiểm tra từng trường có thể chứa mã khách hàng
    const possibleKeys = ['customerId', 'makhachhang', 'makh', 'MaKh', 'Makh', 'id', 'Id', 'ID', 'customerID', 'CustomerID', 'maKH', 'MaKH'];
    
    console.log('🔍 Kiểm tra các trường có thể chứa mã KH:');
    ...
    ['email', 'Email', 'EMAIL'].forEach(key => {
        console.log(`  ${key}: ${currentUser[key]}`);
    });
    
    console.log('📱 Phone fields:');
    ['sdt', 'Sdt', 'SDT', 'phone', 'Phone', 'PHONE'].forEach(key => {
        console.log(`  ${key}: ${currentUser[key]}`);
    });
...
console.log('💡 Sử dụng debugCurrentUser() để kiểm tra thông tin đăng nhập');