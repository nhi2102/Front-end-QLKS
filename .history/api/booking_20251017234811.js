...
async function saveBooking(bookingData) {
    try {
        ...
        try {
            ...
            if (updatedResponse.ok) {
                ...
                if (rawTongTien !== null && rawTongTien !== undefined) {
                    ...
                    console.warn('⚠️ Trigger không tính TONGTIEN (có thể do GIAMGIA = NULL), dùng công thức: DONGIA - GIAMGIA');
                }
                ...
                // Cộng thêm tiền dịch vụ vào tổng tiền cuối cùng
                finalTongTien = finalTongTien + tongTienDichVu;
                console.log('   - TỔNG TIỀN CUỐI CÙNG (phòng + dịch vụ):', finalTongTien);
            } else {
                console.warn('⚠️ Không thể fetch lại DATPHONG, dùng giá trị tạm');
                // Tính tạm: tongTienPhong - giamGia + tongTienDichVu
                finalTongTien = tongTienPhong - finalGiamGia + tongTienDichVu;
            }
        } catch (error) {
            console.warn('⚠️ Lỗi khi fetch DATPHONG:', error.message);
        }
        ...
        const hoaDonPayload = {
            ...
            Tongtien: parseFloat(finalTongTien), // Dùng giá trị từ trigger (đã trừ giảm giá)
            Makh: parseInt(makhachhang),
            Madatphong: parseInt(maDatPhong)
        };
        ...
        // 8. Trả về kết quả
        // Lưu ý: DONGIA, GIAMGIA, TONGTIEN sẽ được trigger tự động cập nhật sau khi insert xong
        ...
        return {
            ...
            data: {
                ...
                tongTienPhong: finalDonGia - finalGiamGia, // Tiền phòng sau giảm
                tongTienDichVu: tongTienDichVu, // Tiền dịch vụ
                tongTien: finalTongTien, // Tổng tiền cuối cùng (phòng + dịch vụ)
                tienThanhToan: finalTongTien, // Thanh toán 100%
                tienCoc: 0,
                conLai: 0
            }
        };
