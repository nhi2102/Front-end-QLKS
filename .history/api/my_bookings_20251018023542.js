...
async function checkReviewStatus(madatphong) {
    try {
        ...
        // Lấy chi tiết đặt phòng để tìm mã phòng liên quan (nếu có)
        let bookingDetails = [];
        try {
            bookingDetails = await getBookingDetails(madatphong);
        } catch (err) {
            console.warn('Không lấy được chi tiết đặt phòng để kiểm tra review:', err.message || err);
        }

        const maphongList = bookingDetails
            .map(d => d.maphong || d.Maphong || d.phongInfo?.maphong || d.phongInfo?.Maphong)
            .filter(Boolean);
        ...
        // Kiểm tra review liên quan đến chính booking này:
        // - Nếu review có field madatphong và trùng madatphong
        // - Hoặc nếu review.maphong nằm trong danh sách phòng của booking
        const hasReviewedForBooking = allReviews.some(review => {
            const reviewerId = review.makh || review.makhachhang || review.makhach;
            if (reviewerId != makhachhang) return false;
            if (review.madatphong && (review.madatphong == madatphong)) return true;
            if (maphongList.length && review.maphong && maphongList.includes(review.maphong)) return true;
            return false;
        });

        if (hasReviewedForBooking) {
            console.log(` Khách hàng ${makhachhang} đã đánh giá cho đặt phòng ${madatphong}`);
            return true;
        }
