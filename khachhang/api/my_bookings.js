...
async function submitReview(reviewData) {
    try {
        ...
        // Lấy thông tin khách hàng đầy đủ từ API (cần để có Hoten)
        let customerFullData = null;
        try {
            const khResponse = await fetch(`${API_BASE}/Khachhangs/${makhachhang}`);
            if (khResponse.ok) {
                customerFullData = await khResponse.json();
                console.log(' Lấy được thông tin khách hàng đầy đủ:', customerFullData);
            } else {
                console.warn(' Không lấy được thông tin khách hàng từ API, thử lấy từ danh sách...');
                // Thử lấy từ danh sách tất cả khách hàng nếu API theo ID không hoạt động
                const khListResponse = await fetch(`${API_BASE}/Khachhangs`);
                if (khListResponse.ok) {
                    const khachhangs = await khListResponse.json();
                    customerFullData = khachhangs.find(kh =>
                        (kh.makh || kh.MaKh || kh.id) === parseInt(makhachhang)
                    );
                    if (customerFullData) {
                        console.log(' Tìm thấy khách hàng trong danh sách:', customerFullData);
                    }
                }
            }
        } catch (error) {
        ...
        // Lấy tên từ localStorage nếu không có từ API
        const customerName = customerFullData ?
            (customerFullData.hoten || customerFullData.Hoten) :
            (currentUser.hoten || currentUser.Hoten || currentUser.name || 'Khách hàng');
        ...
        const reviewPromises = bookingDetails.map(async(chiTiet) => {
            ...
            // Lấy thông tin phòng đầy đủ từ API (cần để có Sophong)
            let roomFullData = null;
            try {
                const roomResponse = await fetch(`${API_BASE}/Phongs/${maphong}`);
                if (roomResponse.ok) {
                    roomFullData = await roomResponse.json();
                    console.log(` Lấy được thông tin phòng ${maphong}:`, roomFullData);
                } else {
                    console.warn(` Không lấy được thông tin phòng ${maphong} từ API`);
                }
            } catch (error) {
                console.error(` Lỗi khi lấy thông tin phòng ${maphong}:`, error);
            }

            // Lấy số phòng từ thông tin phòng hoặc tạo fallback
            const sophong = roomFullData ?
                (roomFullData.sophong || roomFullData.Sophong) :
                (phongInfo.sophong || phongInfo.Sophong || `P${maphong}`);

            // Chuẩn bị dữ liệu theo cấu trúc API Reviews với navigation properties đầy đủ
            const reviewApiData = {
                makh: parseInt(makhachhang),
                maphong: parseInt(maphong),
                sosao: parseInt(sosao),
                danhgia: danhgia,
                // Navigation properties với dữ liệu đầy đủ - cần có tất cả field required
                makhNavigation: {
                    makh: parseInt(makhachhang),
                    hoten: customerName,
                    matkhau: customerFullData ? (customerFullData.matkhau || customerFullData.Matkhau || "review_temp") : "review_temp",
                    email: customerFullData ? (customerFullData.email || customerFullData.Email) : currentUser.email || '',
                    sdt: customerFullData ? (customerFullData.sdt || customerFullData.Sdt) : currentUser.sdt || '',
                    ngaysinh: customerFullData ? (customerFullData.ngaysinh || customerFullData.Ngaysinh) : null,
                    gioitinh: customerFullData ? (customerFullData.gioitinh || customerFullData.Gioitinh) : null,
                    diachi: customerFullData ? (customerFullData.diachi || customerFullData.Diachi) : null
                },
                maphongNavigation: {
                    maphong: parseInt(maphong),
                    sophong: sophong,
                    maloaiphong: roomFullData ? (roomFullData.maloaiphong || roomFullData.Maloaiphong) : 1,
                    trangthai: roomFullData ? (roomFullData.trangthai || roomFullData.Trangthai) : 'Available'
                }
            };

            console.log(' Gửi đánh giá cho phòng', maphong);
            console.log(' - Customer data available:', !!customerFullData);
            console.log(' - Room data available:', !!roomFullData);
            console.log(' - Customer name:', customerName);
            console.log(' - Room number:', sophong);
            console.log(' - Full review data:', JSON.stringify(reviewApiData, null, 2));
